...
async function ensureFrappeDemandSyncTables() {
  await createTableIfMissing(
    "merge_power_demand_sync_mappings",
    `
      CREATE TABLE \`merge_power_demand_sync_mappings\` (
  ...
  await createTableIfMissing(
    "merge_power_demand_sync_runs",
    `
      CREATE TABLE \`merge_power_demand_sync_runs\` (
  ...
  await createTableIfMissing(
    "merge_power_demand_sync_items",
    `
      CREATE TABLE \`merge_power_demand_sync_items\` (
...
async function main() {
  ...
  await ensureFrappeDemandSyncTables();
  await ensureOperationLogTable();
  await renameLegacyTables();
  await addColumnIfMissing(
    "countries",
    "defaultUndertakingUnitId",
    "`defaultUndertakingUnitId` VARCHAR(64) NULL COMMENT 'default undertaking unit for demand requests' AFTER `vatRate`",
  );
  await addColumnIfMissing(
    "countries",
    "defaultCustomerId",
    "`defaultCustomerId` VARCHAR(64) NULL COMMENT 'default customer for demand requests' AFTER `defaultUndertakingUnitId`",
  );
