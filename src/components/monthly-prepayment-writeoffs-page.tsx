...
function renderLinkedValue(row: Row, column: { key: string; type?: string }, returnTo: string) {
  ...
  if (column.key === "contractNo" && row.contractNo) {
    return <Link className="font-medium text-[#1890ff] hover:underline" href={buildDetailRoute(`/finance/prepayment-contracts/${encodeURIComponent(String(row.contractNo))}`, returnTo)}>{value}</Link>;
  }
