  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-medium text-ink">实例结差</h1>
          <p className="mt-1 text-sm text-ink-3">以采购 CAPEX/OPEX、结差汇率和已确认锚定价格版本生成实例结差来源单。</p>
        </div>
      ...
      <div className="flex flex-wrap gap-1 border-b border-line">
        {[["available", "待生成实例结差"], ["settlements", "结差来源单"]].map(([key, label]) => (
          <button
            className={`border-b-2 px-4 py-2 text-sm ${tab === key ? "border-primary text-primary" : "border-transparent text-ink-2 hover:text-primary"}`}
            key={key}
      ...
      {tab === "available" && (
        <>
          <Panel>
            ...
            <StickyTable className="table-scroll table-viewport overflow-auto" tableKey="balance-settlement-available">
                ...
                <tbody>
                  {candidates.map((row) => {
                    ...
                    return <tr className="hover:bg-surface-2" key={row.id}>
                      ...
                      {[[preview?.capexDifferenceTotal, "capex"], [preview?.opexDifferenceTotal, "opex"], [preview?.differenceTotal, "total"]].map(([value, key]) => <td className={`whitespace-nowrap border-b border-r border-line-soft px-3 py-3 ${isNegative(value) ? "text-danger" : ""}`} key={key}>{preview ? formatMoney(value) : "-"}</td>)}
                      <td className={`border-b border-r border-line-soft px-3 py-3 text-xs ${canGenerate ? "text-success-strong" : "text-warning-deep"}`}>{canGenerate ? "可生成" : missing.join("；")}</td>
                    </tr>;
                  })}
                  {!candidates.length && <tr><td className="py-12 text-center text-ink-3" colSpan={18}><TableStateContent empty="暂无可生成的实例结差数据" loading={loading} /></td></tr>}
                </tbody>
              </table>
            </StickyTable>
          ...
          <div className="sticky bottom-4 z-10 border border-info-border bg-info-soft px-4 py-3 shadow-sm">
            <div className="flex flex-wrap items-center gap-3 text-sm text-ink">
              <span>已选择 <b>{selectedRows.length}</b> 条</span><span>预计结差合计 <b className={isNegative(selectedPreviewTotal) ? "text-danger" : ""}>{formatMoney(selectedPreviewTotal)} USD</b></span>
              <Input className="min-w-[220px]" placeholder="结差单名称（可选）" value={draftTitle} onChange={(event) => setDraftTitle(event.target.value)} />
              <label className="grid gap-1 text-xs text-ink-2"><span>结差期间开始</span><Input className="min-w-[150px]" type="date" value={draftPeriodStart} onChange={(event) => setDraftPeriodStart(event.target.value)} /></label>
              <label className="grid gap-1 text-xs text-ink-2"><span>结差期间结束</span><Input className="min-w-[150px]" type="date" value={draftPeriodEnd} onChange={(event) => setDraftPeriodEnd(event.target.value)} /></label>
...
function SettlementDetailPanel({ detail, saving, onClose, onConfirm, onVoid }: { detail: SettlementDetail; saving: boolean; onClose: () => void; onConfirm: () => void; onVoid: () => void }) {
  ...
  return <Panel><div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft p-4"><div><div className="flex items-center gap-2"><h2 className="font-medium text-ink">结差单：{detail.master.settlementNo}</h2><span className={`rounded px-2 py-1 text-xs ${statusClass(detail.master.status)}`}>{asText(detail.master.status)}</span></div><p className="mt-1 text-sm text-ink-3">{asText(detail.master.title)} · 锚定版本：{asText(detail.master.pricingVersionNo) || "手工结差"} · 创建时间：{formatDisplayValue(detail.master.createdAt as never, "datetime")}</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => exportRows(DETAIL_EXPORT_COLUMNS, detail.items, `${detail.master.settlementNo}-明细.xlsx`, "结差明细")}><Download size={15} />导出明细</Button>{detail.master.status === DRAFT && <Button tone="danger" disabled={saving} onClick={onVoid}><XCircle size={15} />作废草稿</Button>}{detail.master.status === DRAFT && <Button tone="success" disabled={saving} onClick={onConfirm}><CheckCircle2 size={15} />确认结差单</Button>}<Button onClick={onClose}>关闭</Button></div></div><div className="grid gap-px border-b border-line-soft bg-line-soft sm:grid-cols-4"><Metric label="明细数量" value={asNumber(detail.master.itemCount)} /><Metric label="CAPEX结差总额" value={formatMoney(detail.master.capexDifferenceTotal)} negative={isNegative(detail.master.capexDifferenceTotal)} /><Metric label="OPEX结差总额" value={formatMoney(detail.master.opexDifferenceTotal)} negative={isNegative(detail.master.opexDifferenceTotal)} /><Metric label="结差合计" value={formatMoney(detail.master.differenceTotal)} negative={isNegative(detail.master.differenceTotal)} /></div><StickyTable className="table-scroll table-viewport overflow-auto" tableKey={`balance-settlement-detail-${detail.master.settlementNo}`}><table className={`w-full min-w-[${hasNonInstanceItems ? "3600" : "2360"}px] border-collapse text-sm`}><thead className="bg-canvas"><tr>{columns.map(([key, label]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={key}>{label}</th>)}</tr></thead><tbody>{detail.items.map((item) => <tr key={asText(item.id)}>{columns.map(([key, , kind]) => <td className={`whitespace-nowrap border-b border-r border-line-soft px-3 py-3 ${key.toLowerCase().includes("difference") && isNegative(item[key]) ? "text-danger" : ""}`} key={key}>{formatValue(item[key], kind as "money" | "date" | "number")}</td>)}</tr>)}</tbody></table></StickyTable></Panel>;
}

function InstanceFormulaDialog({ onClose }: { onClose: () => void }) {
  return (
    <Modal
      footer={<Button tone="primary" onClick={onClose}>知道了</Button>}
      onClose={onClose}
      title="实例结差计算逻辑"
      widthClass="max-w-2xl"
    >
      <div className="space-y-4 text-sm leading-7 text-ink-2"><section><h3 className="font-medium text-ink">实例 CAPEX/OPEX 结差</h3><pre className="mt-2 overflow-auto border border-line-soft bg-surface-2 p-3 font-mono text-xs text-ink">结差 CAPEX 单价 = 采购 CAPEX 单价 / 结差汇率 - CAPEX 锚定单价{`\n`}结差 CAPEX 总额 = 结差 CAPEX 单价 × 数量{`\n`}{`\n`}结差 OPEX 单价 = 采购 OPEX 单价 / 结差汇率 - OPEX 锚定单价{`\n`}结差 OPEX 总额 = 结差 OPEX 单价 × 数量{`\n`}{`\n`}结差合计 = 结差 CAPEX 总额 + 结差 OPEX 总额</pre><p className="mt-2">结差汇率定义为“采购币种 / USD”。采购币种为 USD 时系统固定使用 1；其他币种必须在生成草稿前录入汇率。OPEX 为 0 是有效采购数据。</p></section><section><h3 className="font-medium text-ink">快照与来源确认</h3><p>生成草稿时，系统写入采购价格、锚定版本、锚定单价、汇率及计算结果快照。实例、备件和非实例费用的草稿均在“结差来源单”中统一确认；已确认来源单再由“结差结算单”汇总。</p></section><section><h3 className="font-medium text-ink">非实例费用入口</h3><p>备件、清关费、跨境业务金融税、人力及行政成本和其他 OPEX 请在“非实例费用结差”模块中录入。</p></section></div>
    </Modal>
  );
}

function Metric({ label, value, negative = false }: { label: string; value: string | number; negative?: boolean }) { return <div className="bg-white px-4 py-3"><div className="text-xs text-ink-3">{label}</div><div className={`mt-1 text-base font-medium ${negative ? "text-danger" : "text-ink"}`}>{value}</div></div>; }

function FormulaDialog({ onClose }: { onClose: () => void }) { return <Modal footer={<Button onClick={onClose} tone="primary">知道了</Button>} onClose={onClose} title="结差计算逻辑" widthClass="max-w-2xl"><div className="space-y-4 p-5 text-sm leading-7 text-ink-2"><section><h3 className="font-medium text-ink">实例 CAPEX/OPEX 结差</h3><pre className="mt-2 overflow-auto border border-line-soft bg-surface-2 p-3 font-mono text-xs text-ink">结差 CAPEX 单价 = 采购 CAPEX 单价 / 结差汇率 - CAPEX 锚定单价{`\n`}结差 CAPEX 总额 = 结差 CAPEX 单价 × 数量{`\n`}{`\n`}结差 OPEX 单价 = 采购 OPEX 单价 / 结差汇率 - OPEX 锚定单价{`\n`}结差 OPEX 总额 = 结差 OPEX 单价 × 数量{`\n`}{`\n`}结差合计 = 结差 CAPEX 总额 + 结差 OPEX 总额</pre><p className="mt-2">结差汇率定义为“采购币种 / USD”。采购币种为 USD 时系统固定使用 1；其他币种必须在生成草稿前录入汇率。OPEX 为 0 是有效采购数据。</p></section><section><h3 className="font-medium text-ink">快照与锁定</h3><p>生成草稿时，系统写入采购价格、锚定版本、锚定单价、汇率及全部计算结果快照。确认后不再跟随采购订单或锚定价格版本修改；草稿作废后对应采购明细会释放回待生成清单。</p></section><section><h3 className="font-medium text-ink">备件与非实例费用</h3><p>可手工录入或导入历史 Excel。未设置锚定价时，锚定单价按 0 计算，因此结差等于换算后的采购金额；若存在对比锚定价，可在模板中一起填写。</p></section></div></Modal>; }
