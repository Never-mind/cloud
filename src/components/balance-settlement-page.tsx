    ...
    <div className="space-y-5">
      ...
      {tab === "available" && (
        <>
          ...
          <div className="sticky bottom-4 z-10 border border-[#c6e2ff] bg-info-soft px-4 py-3 shadow-sm">
...
function InstanceFormulaDialog({ onClose }: { onClose: () => void }) {
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true"><div className="max-h-[85vh] w-full max-w-2xl overflow-auto rounded border border-line-soft bg-white shadow-xl"><div className="flex items-center justify-between border-b border-line-soft p-4"><h2 className="font-medium text-ink">实例结差计算逻辑</h2><button className="text-ink-3 hover:text-ink" onClick={onClose} title="关闭"><XCircle size={20} /></button></div><div className="space-y-4 p-5 text-sm leading-7 text-ink-2"><section><h3 className="font-medium text-ink">实例 CAPEX/OPEX 结差</h3><pre className="mt-2 overflow-auto border border-line-soft bg-[#f8fafc] p-3 font-mono text-xs text-ink">结差 CAPEX 单价 = 采购 CAPEX 单价 / 结差汇率 - CAPEX 锚定单价{`\n`}结差 CAPEX 总额 = 结差 CAPEX 单价 × 数量{`\n`}{`\n`}结差 OPEX 单价 = 采购 OPEX 单价 / 结差汇率 - OPEX 锚定单价{`\n`}结差 OPEX 总额 = 结差 OPEX 单价 × 数量{`\n`}{`\n`}结差合计 = 结差 CAPEX 总额 + 结差 OPEX 总额</pre><p className="mt-2">结差汇率定义为“采购币种 / USD”。采购币种为 USD 时系统固定使用 1；其他币种必须在生成草稿前录入汇率。OPEX 为 0 是有效采购数据。</p></section><section><h3 className="font-medium text-ink">快照与来源确认</h3><p>生成草稿时，系统写入采购价格、锚定版本、锚定单价、汇率及计算结果快照。实例、备件和非实例费用的草稿均在“结差来源单”中统一确认；已确认来源单再由“结差结算单”汇总。</p></section><section><h3 className="font-medium text-ink">非实例费用入口</h3><p>备件、清关费、跨境业务金融税、人力及行政成本和其他 OPEX 请在“非实例费用结差”模块中录入。</p></section></div><div className="flex justify-end border-t border-line-soft p-4"><Button tone="primary" onClick={onClose}>知道了</Button></div></div></div>;
}
...
function FormulaDialog({ onClose }: { onClose: () => void }) { return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true"><div className="max-h-[85vh] w-full max-w-2xl overflow-auto rounded border border-line-soft bg-white shadow-xl"><div className="flex items-center justify-between border-b border-line-soft p-4"><h2 className="font-medium text-ink">结差计算逻辑</h2><button className="text-ink-3 hover:text-ink" onClick={onClose} title="关闭"><XCircle size={20} /></button></div><div className="space-y-4 p-5 text-sm leading-7 text-ink-2"><section><h3 className="font-medium text-ink">实例 CAPEX/OPEX 结差</h3><pre className="mt-2 overflow-auto border border-line-soft bg-[#f8fafc] p-3 font-mono text-xs text-ink">结差 CAPEX 单价 = 采购 CAPEX 单价 / 结差汇率 - CAPEX 锚定单价{`\n`}结差 CAPEX 总额 = 结差 CAPEX 单价 × 数量{`\n`}{`\n`}结差 OPEX 单价 = 采购 OPEX 单价 / 结差汇率 - OPEX 锚定单价{`\n`}结差 OPEX 总额 = 结差 OPEX 单价 × 数量{`\n`}{`\n`}结差合计 = 结差 CAPEX 总额 + 结差 OPEX 总额</pre><p className="mt-2">结差汇率定义为“采购币种 / USD”。采购币种为 USD 时系统固定使用 1；其他币种必须在生成草稿前录入汇率。OPEX 为 0 是有效采购数据。</p></section><section><h3 className="font-medium text-ink">快照与锁定</h3><p>生成草稿时，系统写入采购价格、锚定版本、锚定单价、汇率及全部计算结果快照。确认后不再跟随采购订单或锚定价格版本修改；草稿作废后对应采购明细会释放回待生成清单。</p></section><section><h3 className="font-medium text-ink">备件与非实例费用</h3><p>可手工录入或导入历史 Excel。未设置锚定价时，锚定单价按 0 计算，因此结差等于换算后的采购金额；若存在对比锚定价，可在模板中一起填写。</p></section></div><div className="flex justify-end border-t border-line-soft p-4"><Button tone="primary" onClick={onClose}>知道了</Button></div></div></div>; }
