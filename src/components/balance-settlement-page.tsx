            ...
            <StickyTable className="table-scroll overflow-auto" tableKey="balance-settlement-available">
              <table className="w-full min-w-[2060px] border-collapse text-sm">
                <thead className="bg-[#f5f7fa] text-[#303133]"><tr>
                  <th className="w-4 border-b border-r border-[#ebeef5] px-0 py-3 text-center font-medium [&>input]:h-3.5 [&>input]:w-3.5 [&>input]:align-middle"><input type="checkbox" checked={allSelected} onChange={toggleAllCandidates} /></th>
                ...
                <tbody>
                  {candidates.map((row) => {
                    ...
                    return <tr className="hover:bg-[#fafafa]" key={row.id}>
                      <td className="w-4 border-b border-r border-[#ebeef5] px-0 py-3 text-center [&>input]:h-3.5 [&>input]:w-3.5 [&>input]:align-middle"><input type="checkbox" disabled={!canGenerate} checked={selectedIds.includes(row.id)} onChange={() => toggleCandidate(row)} /></td>
