      ...
      {editor ? (
        ...
        <Panel>
          <div className="flex flex-wrap items-end gap-2 border-b border-line-soft p-4">
            <label className="grid gap-1 text-xs text-ink-2">国家<select className="h-9 min-w-[130px] rounded border border-line bg-white px-3 text-sm" value={countryFilter} onChange={(event) => setCountryFilter(event.target.value)}><option value="">全部</option>{countryOptions.map((country) => <option key={country.code} value={country.code}>{country.code} {country.nameZh ? `- ${country.nameZh}` : ""}</option>)}</select></label>
            <label className="grid gap-1 text-xs text-ink-2">状态<select className="h-9 min-w-[110px] rounded border border-line bg-white px-3 text-sm" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">全部</option><option value="草稿">草稿</option><option value="已确认">已确认</option><option value="已废止">已废止</option></select></label>
...
function CurrencySelect({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <select className="h-9 min-w-[82px] rounded border border-line bg-white px-2 text-sm" value={value} onChange={(event) => onChange(event.target.value)}>{CURRENCIES.map((currency) => <option key={currency} value={currency}>{currency}</option>)}</select>; }
