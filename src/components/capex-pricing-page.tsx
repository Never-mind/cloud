...
import { CheckCircle2, ChevronLeft, Download, FileUp, Plus, RefreshCw, Search } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-medium text-ink">CAPEX/OPEX锚定价格</h1>
          <p className="mt-1 text-sm text-ink-3">按国家和价格版本维护锚定价格，确认后锁定完整计算参数与结果。</p>
        </div>
      ...
      {editor ? (
        ...
        <Panel>
          <div className="flex flex-wrap items-end gap-2 border-b border-line-soft p-4">
            <label className="grid gap-1 text-xs text-ink-2">国家<Select className="min-w-[130px]" value={countryFilter} onChange={(event) => setCountryFilter(event.target.value)}><option value="">全部</option>{countryOptions.map((country) => <option key={country.code} value={country.code}>{country.code} {country.nameZh ? `- ${country.nameZh}` : ""}</option>)}</Select></label>
            <label className="grid gap-1 text-xs text-ink-2">状态<Select className="min-w-[110px]" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">全部</option><option value="草稿">草稿</option><option value="已确认">已确认</option><option value="已废止">已废止</option></Select></label>
            <Input placeholder="搜索版本号或来源文件" value={keyword} onChange={(event) => setKeyword(event.target.value)} />
            <Button tone="secondary" onClick={() => void loadVersions()}><Search size={15} />查询</Button>
            <Button onClick={() => void loadVersions()}><RefreshCw size={15} />刷新</Button>
          </div>
          <StickyTable className="table-scroll table-viewport overflow-auto" tableKey="capex-pricing-versions"><table className="min-w-[1280px] border-collapse text-sm"><thead className="bg-canvas text-ink"><tr>{[["versionNo", "价格版本"], ["countryCode", "国家"], ["effectiveDate", "生效日期", "date"], ["status", "状态"], ["itemCount", "明细数量", "number"], ["sourceFileName", "来源文件"], ["confirmedAt", "确认时间", "datetime"], ["createdAt", "创建时间", "datetime"], ["updatedAt", "更新时间", "datetime"]].map(([key, label, type]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={key}>{versionMenu(key, label, type)}</th>)}<th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{versions.map((version) => <tr className="hover:bg-surface-2" key={version.versionId}><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{version.versionNo}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{version.countryCode}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{formatDate(version.effectiveDate)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3"><StatusTag status={String(version.status ?? "")} /></td><td className="border-b border-r border-line-soft px-3 py-3 text-right">{formatDisplayValue(version.itemCount as number, "number")}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{String(version.sourceFileName ?? "-")}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{formatDisplayValue(version.confirmedAt as never, "datetime")}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{formatDisplayValue(version.createdAt as never, "datetime")}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{formatDisplayValue(version.updatedAt as never, "datetime")}</td><td className="border-b border-line-soft px-3 py-3"><button className="text-primary hover:underline" type="button" onClick={() => void loadDetail(version.versionId)}>查看明细</button></td></tr>)}{!versions.length ? <tr><td className="py-12 text-center text-ink-3" colSpan={10}><TableStateContent empty="无数据" loading={loading} /></td></tr> : null}</tbody></table></StickyTable>
        </Panel>
      )}
      ...
      <div className="text-sm text-ink-3">当前视图：{visibleTitle}</div>
    </div>
  );
}

function StatusTag({ status }: { status: string }) {
  const colors: Record<string, string> = { 草稿: "bg-warning-soft text-warning-deep", 已确认: "bg-success-soft text-success-strong", 已废止: "bg-danger-soft text-danger" };
  return <span className={`rounded px-2 py-1 text-xs ${colors[status] ?? "bg-fill-soft text-ink-3"}`}>{status || "-"}</span>;
}

function PricingEditor({
  editor, countries, models, b6Rules, saving, importRef, onChange, onUpdateLine, onUpdateDeviceCode, onAddLine, onRemoveLine, onApplyDefaults, onImport, onDownloadTemplate, onUpload, onSave, onConfirm, onCancel,
}: {
  ...
  onAddLine: () => void; onRemoveLine: (index: number) => void; onApplyDefaults: (index: number, b6Type?: string) => void; onImport: () => void; onDownloadTemplate: () => void; onUpload: (event: React.ChangeEvent<HTMLInputElement>) => void;
  onSave: () => void; onConfirm: () => void; onCancel: () => void;
}) {
  return <Panel><div className="border-b border-line-soft p-4"><div className="mb-4 flex flex-wrap items-center justify-between gap-2"><div className="font-medium text-ink">{editor.versionId ? "编辑价格版本草稿" : "新建价格版本"}</div><div className="flex gap-2"><Button onClick={onCancel}>取消</Button><Button disabled={saving} onClick={onSave}>保存草稿</Button><Button disabled={saving} tone="success" onClick={onConfirm}><CheckCircle2 size={15} />保存并确认</Button></div></div><div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4"><EditorField label="价格版本号"><Input value={editor.versionNo} onChange={(event) => onChange("versionNo", event.target.value)} /></EditorField><EditorField label="国家"><Select value={editor.countryCode} onChange={(event) => onChange("countryCode", event.target.value)}>{countries.map((country) => <option key={country.code} value={country.code}>{country.code} {country.nameZh ? `- ${country.nameZh}` : ""}</option>)}</Select></EditorField><EditorField label="生效日期"><Input type="date" value={editor.effectiveDate} onChange={(event) => onChange("effectiveDate", event.target.value)} /></EditorField><EditorField label="来源文件"><Input value={editor.sourceFileName} onChange={(event) => onChange("sourceFileName", event.target.value)} /></EditorField></div><div className="mt-3"><EditorField label="备注"><Textarea className="w-full" value={editor.notes} onChange={(event) => onChange("notes", event.target.value)} /></EditorField></div></div><div className="flex flex-wrap items-center justify-between gap-2 border-b border-line-soft p-4"><div className="text-sm text-ink-2">价格明细：{editor.lines.length} 条</div><div className="flex flex-wrap gap-2"><input accept=".xlsx,.xls" className="hidden" ref={importRef} type="file" onChange={onUpload} /><Button onClick={onImport}><FileUp size={15} />导入明细</Button><Button onClick={onDownloadTemplate}><Download size={15} />下载导入模板</Button><Button tone="primary" onClick={onAddLine}><Plus size={15} />新增明细</Button></div></div><StickyTable className="table-scroll table-viewport overflow-auto" tableKey="capex-pricing-editor-lines"><table className="min-w-[2700px] border-collapse text-sm"><thead className="bg-canvas"><tr>{["设备编码", "B6类型", "整机价格（不含VAT）", "整机价格币种", "合同币种", "换算汇率", "设备VAT（%）", "服务VAT（%）", "Onsite/RMA（%）", "资金年利率（%）", "资金占用月数", "备件占用月数", "海外备件服务", "备件费率（%）", "备件结算方式", "运保清关（%）", "总代过手（%）", "其他税费（%）", "巴西服务税（%）", "操作"].map((label) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={label}>{label}</th>)}</tr></thead><tbody>{editor.lines.map((line, index) => <PricingEditorRow key={`${line.id ?? "new"}-${index}`} line={line} index={index} models={models} b6Rules={b6Rules} onUpdateLine={onUpdateLine} onUpdateDeviceCode={onUpdateDeviceCode} onApplyDefaults={onApplyDefaults} onRemoveLine={onRemoveLine} />)}{!editor.lines.length ? <tr><td className="py-12 text-center text-ink-3" colSpan={20}><EmptyState title="请新增或导入价格明细" /></td></tr> : null}</tbody></table></StickyTable></Panel>;
}
...
function PricingEditorRow({ line, index, models, b6Rules, onUpdateLine, onUpdateDeviceCode, onApplyDefaults, onRemoveLine }: { line: EditableLine; index: number; models: InstanceModel[]; b6Rules: B6Rule[]; onUpdateLine: (index: number, field: keyof EditableLine, value: string | number | boolean) => void; onUpdateDeviceCode: (index: number, deviceCode: string) => void; onApplyDefaults: (index: number, b6Type?: string) => void; onRemoveLine: (index: number) => void }) {
  ...
  return <tr><td className="border-b border-r border-line-soft p-2"><SearchSelect
        className="min-w-[190px]"
        emptyText="没有匹配的设备编码"
        options={models.map((model) => ({
          value: String(model.deviceCode ?? ""),
          label: String(model.deviceCode ?? ""),
          hint: [model.modelCode, model.nameEn, model.b6Type].filter(Boolean).join(" / "),
        }))}
        placeholder="输入或搜索编码"
        value={String(line.deviceCode ?? "")}
        onChange={(value) => onUpdateDeviceCode(index, value)}
      /></td><td className="border-b border-r border-line-soft p-2"><div><SearchSelect
        className="min-w-[160px]"
        emptyText="没有匹配的B6类型"
        options={b6Rules.map((rule) => ({
          value: String(rule.b6Type ?? ""),
          label: String(rule.b6Type ?? ""),
          hint: rule.alias ? `${rule.alias} / ${rule.scope ?? ""}` : String(rule.scope ?? ""),
        }))}
        placeholder="选择或搜索B6类型"
        value={String(line.b6Type ?? "")}
        onChange={(value) => {
          onUpdateLine(index, "b6Type", value);
          if (value.trim()) onApplyDefaults(index, value);
        }}
      />{rule ? <div className="mt-1 max-w-[180px] truncate text-xs text-ink-3" title={`${rule.scope ?? ""} ${rule.notes ?? ""}`}>{manualAdjusted ? "人工调整" : rule.scope || "规则已套用"}</div> : <div className="mt-1 text-xs text-danger">未匹配规则</div>}</div></td><td className="border-b border-r border-line-soft p-2"><NumericInput value={line.baseCapexPrice} onChange={(value) => onUpdateLine(index, "baseCapexPrice", value)} /></td><td className="border-b border-r border-line-soft p-2"><CurrencySelect value={line.priceCurrency} onChange={(value) => onUpdateLine(index, "priceCurrency", value)} /></td><td className="border-b border-r border-line-soft p-2"><CurrencySelect value={line.contractCurrency} onChange={(value) => onUpdateLine(index, "contractCurrency", value)} /></td><td className="border-b border-r border-line-soft p-2"><NumericInput value={line.exchangeRate} onChange={(value) => onUpdateLine(index, "exchangeRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.deviceVatRate} onChange={(value) => onUpdateLine(index, "deviceVatRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.serviceVatRate} onChange={(value) => onUpdateLine(index, "serviceVatRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.onsiteRmaRate} onChange={(value) => onUpdateLine(index, "onsiteRmaRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.fundingAnnualRate} onChange={(value) => onUpdateLine(index, "fundingAnnualRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><NumericInput value={line.fundingMonths} onChange={(value) => onUpdateLine(index, "fundingMonths", value)} /></td><td className="border-b border-r border-line-soft p-2"><NumericInput value={line.spareOccupancyMonths} onChange={(value) => onUpdateLine(index, "spareOccupancyMonths", value)} /></td><td className="border-b border-r border-line-soft p-2"><Select className="min-w-[106px]" value={line.overseasSpareServiceAvailable === "" ? "" : line.overseasSpareServiceAvailable ? "1" : "0"} onChange={(event) => onUpdateLine(index, "overseasSpareServiceAvailable", event.target.value === "" ? "" : event.target.value === "1")}><option value="">待确认</option><option value="1">提供</option><option value="0">不提供</option></Select></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.spareRate} onChange={(value) => onUpdateLine(index, "spareRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><Input className="min-w-[130px]" value={line.spareSettlementMethod} onChange={(event) => onUpdateLine(index, "spareSettlementMethod", event.target.value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.transportClearanceRate} onChange={(value) => onUpdateLine(index, "transportClearanceRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.handlingRate} onChange={(value) => onUpdateLine(index, "handlingRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.otherTaxRate} onChange={(value) => onUpdateLine(index, "otherTaxRate", value)} /></td><td className="border-b border-r border-line-soft p-2"><PercentageInput value={line.brazilServiceTaxRate} onChange={(value) => onUpdateLine(index, "brazilServiceTaxRate", value)} /></td><td className="border-b border-line-soft p-2"><div className="flex gap-2"><button className="text-primary hover:underline" type="button" onClick={() => onApplyDefaults(index, line.b6Type)}>套用规则</button><button className="text-danger hover:underline" type="button" onClick={() => onRemoveLine(index)}>删除</button></div></td></tr>;
}
...
function PricingDetailView({ detail, detailKeyword, saving, onBack, onEdit, onConfirm, onClone, onExport, onOpenCalculation, onPageChange, onPageSizeChange, onSearch }: { detail: PricingDetail; detailKeyword: string; saving: boolean; onBack: () => void; onEdit: () => void; onConfirm: () => void; onClone: () => void; onExport: () => void; onOpenCalculation: (item: PricingItem) => void; onPageChange: (page: number) => void; onPageSizeChange: (pageSize: number) => void; onSearch: (value: string) => void }) {
  ...
  return <><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><button className="text-ink-2 hover:text-primary" title="返回价格版本" type="button" onClick={onBack}><ChevronLeft size={21} /></button><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-medium text-ink">{detail.version.versionNo} · {detail.version.countryCode}</h2><StatusTag status={String(detail.version.status ?? "")} /></div><p className="mt-1 text-sm text-ink-3">生效日期 {formatDate(detail.version.effectiveDate)} · 共 {detail.total} 条明细 · 确认后锁定</p></div></div><div className="flex flex-wrap gap-2"><Button onClick={onExport}><Download size={15} />导出全部明细</Button><Button onClick={onClone}>复制为新版本</Button>{editable ? <Button onClick={onEdit}>编辑草稿</Button> : null}{editable ? <Button disabled={saving} tone="success" onClick={onConfirm}><CheckCircle2 size={15} />确认版本</Button> : null}</div></div><Panel><div className="flex flex-wrap items-center justify-between gap-2 border-b border-line-soft p-4"><span className="text-sm text-ink-2">设备锚定价格明细</span><Input placeholder="搜索设备编码、机型、英文名称或B6类型" value={detailKeyword} onChange={(event) => onSearch(event.target.value)} /></div><StickyTable className="table-scroll table-viewport overflow-auto" tableKey={`capex-pricing-detail-${detail.version.versionId}`}><table className="min-w-[1380px] border-collapse text-sm"><thead className="bg-canvas text-ink"><tr>{detailColumns.map(([key, label]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={key}>{label}</th>)}<th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{detail.rows.map((row) => <tr className="hover:bg-surface-2" key={row.id}>{detailColumns.map(([key, , type]) => <td className={`whitespace-nowrap border-b border-r border-line-soft px-3 py-3 ${type === "money" || type === "number" ? "text-right" : ""}`} key={key}>{formatDisplayValue(row[key] as string | number | null, type)}</td>)}<td className="border-b border-line-soft px-3 py-3"><button className="whitespace-nowrap text-primary hover:underline" type="button" onClick={() => onOpenCalculation(row)}>查看计算逻辑</button></td></tr>)}{!detail.rows.length ? <tr><td className="py-12 text-center text-ink-3" colSpan={detailColumns.length + 1}><EmptyState title="无数据" /></td></tr> : null}</tbody></table></StickyTable><PaginationBar page={detail.page} pageSize={detail.pageSize} total={detail.total} onPageChange={onPageChange} onPageSizeChange={onPageSizeChange} /></Panel></>;
}

function CalculationDialog({ data, onClose }: { data: CalculationData; onClose: () => void }) {
  const item = data.item;
  const row = (label: string, formula: string, value: string | number, type: "money" | "percentage" | "number" = "money") => <tr key={label}><td className="border-b border-r border-line-soft px-3 py-3 font-medium">{label}</td><td className="border-b border-r border-line-soft px-3 py-3 font-mono text-xs text-ink-2">{formula}</td><td className="border-b border-line-soft px-3 py-3 text-right">{typeof value === "string" ? value : formatDisplayValue(value, type)}</td></tr>;
  return <Modal description={`${String(item.versionNo ?? "")} · ${String(item.countryCode ?? "")} · ${String(item.b6Type ?? "")} · ${String(item.nameEn ?? "")}`} onClose={onClose} title={`计算逻辑 · ${String(item.deviceCode ?? "")}`} widthClass="max-w-5xl"><div className="grid gap-4 xl:grid-cols-3"><div className="xl:col-span-2"><div className="mb-2 text-sm font-medium text-ink">参数与计算过程</div><div className="table-scroll overflow-auto"><table className="w-full min-w-[760px] border-collapse text-sm"><tbody>{row("整机价格", "手工录入", `${formatDisplayValue(item.baseCapexPrice as number, "money")} ${String(item.priceCurrency ?? "")}`, "money")}{row("资金占用比例", "资金占用年利率 × 资金占用月数 ÷ 12", numberValue(item.fundingRatio), "percentage")}{row("资金占用金额", "整机价格 × 资金占用比例", numberValue(item.fundingAmount))}{row("CAPEX 合计", "整机价格 × (1 + 资金占用比例 + Onsite/RMA费率)", numberValue(item.capexTotal))}{row("DDP价格", "CAPEX合计 × (1 + 运保清关费率) × (1 + 总代过手费率 + 其他税费率)", numberValue(item.ddpPrice))}{row("OPEX金额", "DDP价格 - CAPEX合计", numberValue(item.opexAmount))}{row("原始 CAPEX 锚定", "CAPEX合计 × 整机价转合同汇率", `${formatDisplayValue(item.rawCapexAnchorUsd as number, "money")} USD`)}{row("原始 OPEX 锚定", "OPEX金额 × 整机价转合同汇率", `${formatDisplayValue(item.rawOpexAnchorUsd as number, "money")} USD`)}{row("最终 CAPEX 锚定", item.countryCode === "BR" || item.countryCode === "巴西" ? "原始CAPEX锚定 ÷ (1 + 巴西服务税率)" : "原始CAPEX锚定", `${formatDisplayValue(item.capexAnchorUsd as number, "money")} USD`)}{row("最终 OPEX 锚定", item.countryCode === "BR" || item.countryCode === "巴西" ? "原始OPEX锚定 ÷ (1 + 巴西服务税率)" : "原始OPEX锚定", `${formatDisplayValue(item.opexAnchorUsd as number, "money")} USD`)}</tbody></table></div></div><div><div className="mb-2 text-sm font-medium text-ink">取数快照</div><div className="space-y-2 border border-line-soft p-3 text-sm"><div><span className="text-ink-3">实例型号</span><div>{String(item.modelCode ?? "")} / {String(item.nameEn ?? "")}</div></div><div><span className="text-ink-3">设备 VAT</span><div>{formatDisplayValue(item.deviceVatRate as number, "percentage")}</div></div><div><span className="text-ink-3">服务 VAT</span><div>{formatDisplayValue(item.serviceVatRate as number, "percentage")}</div></div><div><span className="text-ink-3">巴西服务税率</span><div>{formatDisplayValue(item.brazilServiceTaxRate as number, "percentage")}</div></div><div><span className="text-ink-3">来源</span><div>实例型号管理、国家管理、价格版本明细</div></div></div></div></div><div className="border-t border-line-soft p-5"><div className="mb-2 text-sm font-medium text-ink">历史版本对比</div><div className="table-scroll overflow-auto"><table className="min-w-full border-collapse text-sm"><thead className="bg-canvas"><tr><th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">版本</th><th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">生效日期</th><th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-right font-medium">CAPEX锚定（USD）</th><th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-right font-medium">OPEX锚定（USD）</th></tr></thead><tbody>{data.history.map((history) => <tr key={String(history.versionId)}><td className="border-b border-r border-line-soft px-3 py-2">{String(history.versionNo ?? "")}</td><td className="border-b border-r border-line-soft px-3 py-2">{formatDate(history.effectiveDate)}</td><td className="border-b border-r border-line-soft px-3 py-2 text-right">{formatDisplayValue(history.capexAnchorUsd as number, "money")}</td><td className="border-b border-line-soft px-3 py-2 text-right">{formatDisplayValue(history.opexAnchorUsd as number, "money")}</td></tr>)}{!data.history.length ? <tr><td className="py-8 text-center text-ink-3" colSpan={4}>无历史版本数据</td></tr> : null}</tbody></table></div></div></Modal>;
}
...
function CurrencySelect({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <Select className="min-w-[82px]" value={value} onChange={(event) => onChange(event.target.value)}>{CURRENCIES.map((currency) => <option key={currency} value={currency}>{currency}</option>)}</Select>; }
