...
export function CapexPricingPage() {
  ...
  async function loadVersions(nextPage = versionPage, nextPageSize = versionPageSizeRef.current) {
    setLoading(true);
    try {
    ...
    <div className="space-y-5">
      ...
      {editor ? (
        ...
        <Panel>
          ...
          <div className="table-scroll overflow-auto"><table className="min-w-full border-collapse text-sm"><thead className="bg-[#f5f7fa] text-[#303133]"><tr><th className="border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium">价格版本</th><th className="border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium">国家</th><th className="border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium">生效日期</th><th className="border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium">状态</th><th className="border-b border-r border-[#ebeef5] px-3 py-3 text-right font-medium">明细数量</th><th className="border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium">来源文件</th><th className="border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium">确认日期</th><th className="border-b border-[#ebeef5] px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{versions.map((version) => <tr className="hover:bg-[#fafafa]" key={version.versionId}><td className="border-b border-r border-[#ebeef5] px-3 py-3">{version.versionNo}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{version.countryCode}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{formatDate(version.effectiveDate)}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3"><StatusTag status={String(version.status ?? "")} /></td><td className="border-b border-r border-[#ebeef5] px-3 py-3 text-right">{formatDisplayValue(version.itemCount as number, "number")}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{String(version.sourceFileName ?? "-")}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{formatDate(version.confirmedAt)}</td><td className="border-b border-[#ebeef5] px-3 py-3"><button className="text-[#1890ff] hover:underline" type="button" onClick={() => void loadDetail(version.versionId)}>查看明细</button></td></tr>)}{!versions.length ? <tr><td className="py-12 text-center text-[#909399]" colSpan={8}>{loading ? "加载中..." : "无数据"}</td></tr> : null}</tbody></table></div>
        </Panel>
      )}
