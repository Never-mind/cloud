    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        <div className="mr-auto">
          <h1 className="text-xl font-medium text-ink">客户PO</h1>
          <p className="mt-1 text-sm text-ink-3">客户采购订单主单与产品明细。</p>
        </div>
      </div>
      <Panel>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-line-soft p-3">
          ...
          <a className="inline-flex h-8 shrink-0 items-center gap-1 whitespace-nowrap rounded border border-warning-deep bg-white px-3 text-sm text-[#8a5200] transition-colors hover:border-[#d48806] hover:bg-warning-soft" href={`/api/entities/customer-pos/export?status=${encodeURIComponent(status)}`}><FileDown size={14} />导出 Excel</a>
        </div>
          ...
          <table className="min-w-[1500px] border-collapse text-sm">
            ...
            <tbody>
              {loading ? <tr><td className="px-4 py-12 text-center text-ink-3" colSpan={listFields.length + 1}><TableSkeleton /></td></tr> : rows.map((row) => {
                ...
                return <tr className="hover:bg-surface-2" key={id}>{listFields.map((field) => <td className="max-w-[240px] truncate whitespace-nowrap border-b border-r border-line-soft px-3 py-3" key={field.key}>{field.key === "poNo" ? <button className="text-primary hover:underline" type="button" onClick={() => openRoute(`/customer-pos/${encodeURIComponent(id)}?returnTo=%2Fcustomer-pos`, "客户PO明细")}>{String(row[field.key] ?? "-")}</button> : field.key === "status" ? <StatusTag status={String(row[field.key] ?? "draft")} label={formatPoStatus(row[field.key])} /> : formatPoListValue(row[field.key], "type" in field ? field.type : undefined)}</td>)}<td className="sticky right-0 whitespace-nowrap border-b border-line-soft bg-white px-3 py-3"><button className="inline-flex h-8 w-8 items-center justify-center text-ink-2 hover:text-primary" type="button" aria-label="查看" title="查看" onClick={() => openRoute(`/customer-pos/${encodeURIComponent(id)}?returnTo=%2Fcustomer-pos`, "客户PO明细")}><Eye size={16} /></button>{String(row.status ?? "draft") !== "confirmed" ? <button className="ml-2 inline-flex h-8 w-8 items-center justify-center text-danger hover:text-danger-strong" type="button" aria-label="删除" title="删除草稿" onClick={() => void deletePo(id, String(row.poNo ?? ""))}><Trash2 size={16} /></button> : null}</td></tr>;
              })}
              {!loading && !rows.length ? <tr><td className="px-4 py-12 text-center text-ink-3" colSpan={listFields.length + 1}>暂无客户PO</td></tr> : null}
            </tbody>
          </table>
        </StickyTable>
...
export function CustomerPoDetailPage({ config, id }: { config: EntityConfig; id: string }) {
  ...
  if (loading) return <Panel className="p-10 text-center text-sm text-ink-3">正在加载客户PO...</Panel>;
  if (!master) return <Panel className="p-10 text-center text-sm text-danger">{error || "未找到客户PO"}</Panel>;

  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <Button onClick={goBack}><ArrowLeft size={15} />返回列表</Button>
        <div><h1 className="text-xl font-medium text-ink">客户PO明细：{String(masterDraft.poNo ?? id)}</h1><p className="mt-1 text-sm text-ink-3">维护客户PO主单和产品明细，明细允许先自由录入，后续再匹配产品主档。</p></div>
      ...
      {notice ? <div className="border border-success-border bg-success-soft px-4 py-3 text-sm text-success-strong">{notice}</div> : null}

      <Panel>
        <div className="border-b border-line-soft px-4 py-3 font-medium text-ink">主单基础信息</div>
        <div className="grid gap-x-5 gap-y-4 p-4 md:grid-cols-2 lg:grid-cols-4">
          <label><span className="mb-1 block text-xs text-ink-2">客户PO号 <b className="text-danger">*</b></span><Input className="w-full" disabled={!editing} value={String(masterDraft.poNo ?? "")} onChange={(event) => updateMaster("poNo", event.target.value)} /></label>
          <label><span className="mb-1 block text-xs text-ink-2">项目名称</span><Input className="w-full" disabled={!editing} value={String(masterDraft.projectName ?? "")} onChange={(event) => updateMaster("projectName", event.target.value)} /></label>
          ...
          <label><span className="mb-1 block text-xs text-ink-2">PO日期 <b className="text-danger">*</b></span><Input className="w-full" disabled={!editing} type="date" value={formatDateInputValue(masterDraft.poDate)} onChange={(event) => updateMaster("poDate", event.target.value)} /></label>
          <label><span className="mb-1 block text-xs text-ink-2">交付日期</span><Input className="w-full" disabled={!editing} type="date" value={formatDateInputValue(masterDraft.deliveryDate)} onChange={(event) => updateMaster("deliveryDate", event.target.value)} /></label>
          <label><span className="mb-1 block text-xs text-ink-2">币种</span><Input className="w-full" disabled={!editing} value={String(masterDraft.currency ?? "USD")} onChange={(event) => updateMaster("currency", event.target.value)} /></label>
          <label><span className="mb-1 block text-xs text-ink-2">状态</span><div className="flex h-9 items-center text-sm text-ink-2">{formatPoStatus(masterDraft.status)}</div></label>
          <label className="md:col-span-2 lg:col-span-4"><span className="mb-1 block text-xs text-ink-2">备注</span><Textarea className="w-full" disabled={!editing} value={String(masterDraft.remark ?? "")} onChange={(event) => updateMaster("remark", event.target.value)} /></label>
        </div>
      </Panel>
...
function CustomerPoItemRow({ editing, row, onChange, onRemove }: { editing: boolean; row: Row; onChange: (id: string, key: string, value: Value) => void; onRemove: (row: Row) => void }) {
  ...
  return <tr className="hover:bg-surface-2">
    <td className="border-b border-r border-line-soft px-2 py-2 text-ink-2">{formatDisplayValue(row.lineNo, "number")}</td>
    <td className="border-b border-r border-line-soft px-2 py-2">{input("customerSku")}</td>
    <td className="border-b border-r border-line-soft px-2 py-2">{input("customerProductName")}</td>
    <td className="border-b border-r border-line-soft px-2 py-2">{input("customerBrand")}</td>
    <td className="border-b border-r border-line-soft px-2 py-2">{input("customerSpec")}</td>
    <td className="border-b border-r border-line-soft px-2 py-2">{input("quantity", "number", "w-full !min-w-0")}</td>
    <td className="border-b border-r border-line-soft px-2 py-2">{input("unit", "text", "w-full !min-w-0")}</td>
