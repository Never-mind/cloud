...
export function BalanceFinalSettlementPage() {
  ...
  async function loadFinals(page = finalPage, pageSize = finalPageSize) {
    const data = await fetchJson<{ rows: FinalSettlement[]; total: number; page: number; pageSize: number }>(`/api/balance-settlements/finals?page=${page}&pageSize=${pageSize}`);
    setFinals(data.rows ?? []);
  ...
  async function searchSources(page = sourcePage, pageSize = sourcePageSize) {
  ...
  function exportFinals() {
    exportRowsToXlsx({
      ...
      columns: [
        ...
        ["differenceTotal", "结差合计"], ["notes", "备注"], ["confirmedAt", "确认日期"], ["createdAt", "创建日期"],
  ...
  return <div className="space-y-4">
    <Panel className="overflow-hidden">
      ...
      <div className="table-scroll overflow-auto">
        <table className="w-full min-w-[1400px] border-collapse text-sm"><thead className="bg-[#f5f7fa]"><tr>{["", "来源结差单号", "名称", "结差类型", "国家", "币种", "结差期间", "明细数", "CAPEX结差", "OPEX结差", "结差合计"].map((label) => <th className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium" key={label || "select"}>{label}</th>)}</tr></thead><tbody>
    ...
    <Panel className="overflow-hidden">
      ...
      <div className="table-scroll overflow-auto"><table className="w-full min-w-[1450px] border-collapse text-sm"><thead className="bg-[#f5f7fa]"><tr>{["结算单号", "名称", "国家", "币种", "结差期间", "状态", "来源单数", "明细数", "CAPEX结差", "OPEX结差", "结差合计", "确认日期", "创建日期"].map((label) => <th className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium" key={label}>{label}</th>)}</tr></thead><tbody>{finals.map((row) => <tr key={row.finalSettlementNo} className="hover:bg-[#fafafa]"><td className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3"><button className="text-[#1890ff] hover:underline" onClick={() => void openFinal(row.finalSettlementNo)}>{row.finalSettlementNo}</button></td><td className="max-w-[240px] truncate border-b border-r border-[#ebeef5] px-3 py-3">{text(row.title) || "-"}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{text(row.countryCode)}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{text(row.currency)}</td><td className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3">{dateValue(row.periodStart)} 至 {dateValue(row.periodEnd)}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3"><span className={`rounded px-2 py-1 text-xs ${statusClass(row.status)}`}>{text(row.status)}</span></td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{numberValue(row.sourceCount)}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3">{numberValue(row.itemCount)}</td>{["capexDifferenceTotal", "opexDifferenceTotal", "differenceTotal"].map((key) => <td className={`border-b border-r border-[#ebeef5] px-3 py-3 text-right ${isNegative(row[key]) ? "text-[#f56c6c]" : ""}`} key={key}>{money(row[key])}</td>)}<td className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3">{dateValue(row.confirmedAt)}</td><td className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3">{dateValue(row.createdAt)}</td></tr>)}{!finals.length && <tr><td colSpan={13} className="py-12 text-center text-[#909399]">暂无结差结算单</td></tr>}</tbody></table></div><PaginationBar page={finalPage} pageSize={finalPageSize} total={finalTotal} onPageChange={(next) => void loadFinals(next)} onPageSizeChange={(next) => { setFinalPageSize(next); void loadFinals(1, next); }} />
    </Panel>
