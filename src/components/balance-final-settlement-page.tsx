...
import { formatDisplayValue, formatMoneyValue, isDateTimeFieldKey } from "@/lib/display-format";
...
function statusClass(status: unknown) {
  if (status === CONFIRMED) return "bg-success-soft text-success-strong";
  if (status === VOIDED) return "bg-fill-soft text-ink-3";
  ...
  function exportFinals() {
    exportRowsToXlsx({
      ...
      columns: [
        ...
        ["differenceTotal", "结差合计"], ["notes", "备注"], ["confirmedAt", "确认时间"], ["createdAt", "创建时间"], ["updatedAt", "更新时间"],
      ].map(([key, label]) => ({
        key,
        label,
        format: (value) => isDateTimeFieldKey(key)
          ? formatDisplayValue(value as never, "datetime")
          : key.endsWith("At") || key.startsWith("period")
            ? dateValue(value)
  ...
  return <div className="space-y-4">
    <Panel>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft p-4">
        <div><h1 className="font-medium text-ink">结差结算单</h1><p className="mt-1 text-sm text-ink-3">汇集同国家、同结差期间、同币种的已确认实例结差与非实例费用结差。</p></div>
        <Button onClick={() => void loadFinals()}><RefreshCw size={15} />刷新列表</Button>
      </div>
      <div className="grid gap-3 border-b border-line-soft p-4 md:grid-cols-2 xl:grid-cols-4">
        <Input placeholder="结算单名称（可选）" value={title} onChange={(event) => setTitle(event.target.value)} />
        <Select value={countryCode} onChange={(event) => { setCountryCode(event.target.value); setSources([]); setSelectedNos([]); }}><option value="">选择国家</option>{countries.map((country) => <option key={country.code} value={country.code}>{country.code} {country.nameZh ? `- ${country.nameZh}` : ""}</option>)}</Select>
        <Select value={currency} onChange={(event) => { setCurrency(event.target.value); setSources([]); setSelectedNos([]); }}>{CURRENCIES.map((item) => <option key={item} value={item}>{item}</option>)}</Select>
      ...
      <StickyTable className="table-scroll table-viewport overflow-auto" tableKey="balance-final-sources">
        <table className="w-full min-w-[1400px] border-collapse text-sm"><thead className="bg-canvas"><tr>{[["", ""], ["settlementNo", "来源结差单号"], ["title", "名称"], ["itemTypes", "结差类型"], ["countryCode", "国家"], ["currency", "币种"], ["periodStart", "结差开始"], ["periodEnd", "结差结束"], ["itemCount", "明细数"], ["capexDifferenceTotal", "CAPEX结差"], ["opexDifferenceTotal", "OPEX结差"], ["differenceTotal", "结差合计"]].map(([key, label]) => <th className={key ? "whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" : "table-select-cell border-b border-r border-line-soft py-3 text-center font-medium [&>input]:h-4 [&>input]:w-4 [&>input]:align-middle"} key={key || "select"}>{key ? <TableColumnMenu column={{ key, label, sortable: true, filterable: true }} sortOrder={sourceSortField === key ? sourceSortOrder : ""} filterValues={sourceFilters[key] ?? []} loadOptions={(keyword) => fetchTableFilterOptions("/api/balance-settlements/finals/available", key, keyword, { countryCode, currency, periodStart, periodEnd }, sourceFilters)} onSort={(order) => updateSourceQuery(key, { order })} onFilter={(values) => updateSourceQuery(key, { values })} /> : label}</th>)}</tr></thead><tbody>
          {sources.map((source) => <tr key={source.settlementNo} className="hover:bg-surface-2"><td className="table-select-cell border-b border-r border-line-soft py-3 text-center [&>input]:h-4 [&>input]:w-4 [&>input]:align-middle"><input type="checkbox" checked={selectedNos.includes(source.settlementNo)} onChange={() => { const sourceNo = text(source.settlementNo); setSelectedNos((current) => current.includes(sourceNo) ? current.filter((value) => value !== sourceNo) : [...current, sourceNo]); setSelectedSourcesByNo((current) => { if (current[sourceNo]) { const { [sourceNo]: _removed, ...next } = current; return next; } return { ...current, [sourceNo]: source }; }); }} /></td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-primary">{source.settlementNo}</td><td className="max-w-[240px] truncate border-b border-r border-line-soft px-3 py-3">{text(source.title) || "-"}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(source.itemTypes) || "-"}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(source.countryCode)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(source.currency)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{dateValue(source.periodStart)} 至 {dateValue(source.periodEnd)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{numberValue(source.itemCount)}</td>{["capexDifferenceTotal", "opexDifferenceTotal", "differenceTotal"].map((key) => <td className={`border-b border-r border-line-soft px-3 py-3 text-right ${isNegative(source[key]) ? "text-danger" : ""}`} key={key}>{money(source[key])}</td>)}</tr>)}
          {!sources.length && <tr><td colSpan={11} className="py-12 text-center text-ink-3"><TableStateContent empty="请先按国家、期间和币种查询可汇总的已确认来源单" loading={loading} /></td></tr>}
        </tbody></table>
      </StickyTable>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-t border-line-soft p-4"><div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-ink-2"><span>已选来源单：<b className="text-ink">{selectedSources.length}</b></span><span>明细数量：<b className="text-ink">{selectedTotals.count}</b></span><span>CAPEX：<b className={isNegative(selectedTotals.capex) ? "text-danger" : "text-ink"}>{money(selectedTotals.capex)}</b></span><span>OPEX：<b className={isNegative(selectedTotals.opex) ? "text-danger" : "text-ink"}>{money(selectedTotals.opex)}</b></span><span>合计：<b className={isNegative(selectedTotals.all) ? "text-danger" : "text-ink"}>{money(selectedTotals.all)} {currency}</b></span></div><Button tone="primary" disabled={saving || !selectedNos.length} onClick={() => void createDraft()}><ListChecks size={15} />生成结算草稿</Button></div>
    </Panel>

    <Panel>
      <div className="flex items-center justify-between gap-3 border-b border-line-soft p-4"><div><h2 className="font-medium text-ink">结差结算单列表</h2><p className="mt-1 text-sm text-ink-3">已确认结算单为最终汇总口径；草稿作废后会释放已占用的来源单。</p></div><Button onClick={exportFinals}><Download size={15} />导出</Button></div>
      <StickyTable className="table-scroll table-viewport overflow-auto" tableKey="balance-final-settlements"><table className="w-full min-w-[1550px] border-collapse text-sm"><thead className="bg-canvas"><tr>{[["finalSettlementNo", "结算单号"], ["title", "名称"], ["countryCode", "国家"], ["currency", "币种"], ["periodStart", "结差开始"], ["periodEnd", "结差结束"], ["status", "状态"], ["sourceCount", "来源单数"], ["itemCount", "明细数"], ["capexDifferenceTotal", "CAPEX结差"], ["opexDifferenceTotal", "OPEX结差"], ["differenceTotal", "结差合计"], ["confirmedAt", "确认时间"], ["createdAt", "创建时间"], ["updatedAt", "更新时间"]].map(([key, label]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={key}><TableColumnMenu column={{ key, label, sortable: true, filterable: true }} sortOrder={finalSortField === key ? finalSortOrder : ""} filterValues={finalFilters[key] ?? []} loadOptions={(keyword) => fetchTableFilterOptions("/api/balance-settlements/finals", key, keyword, {}, finalFilters)} onSort={(order) => updateFinalQuery(key, { order })} onFilter={(values) => updateFinalQuery(key, { values })} /></th>)}</tr></thead><tbody>{finals.map((row) => <tr key={row.finalSettlementNo} className="hover:bg-surface-2"><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3"><button className="text-primary hover:underline" onClick={() => void openFinal(row.finalSettlementNo)}>{row.finalSettlementNo}</button></td><td className="max-w-[240px] truncate border-b border-r border-line-soft px-3 py-3">{text(row.title) || "-"}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(row.countryCode)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(row.currency)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{dateValue(row.periodStart)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{dateValue(row.periodEnd)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3"><span className={`rounded px-2 py-1 text-xs ${statusClass(row.status)}`}>{text(row.status)}</span></td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{numberValue(row.sourceCount)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{numberValue(row.itemCount)}</td>{["capexDifferenceTotal", "opexDifferenceTotal", "differenceTotal"].map((key) => <td className={`border-b border-r border-line-soft px-3 py-3 text-right ${isNegative(row[key]) ? "text-danger" : ""}`} key={key}>{money(row[key])}</td>)}<td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{formatDisplayValue(row.confirmedAt as never, "datetime")}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{formatDisplayValue(row.createdAt as never, "datetime")}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{formatDisplayValue(row.updatedAt as never, "datetime")}</td></tr>)}{!finals.length && <tr><td colSpan={15} className="py-12 text-center text-ink-3">暂无结差结算单</td></tr>}</tbody></table></StickyTable><PaginationBar page={finalPage} pageSize={finalPageSize} total={finalTotal} onPageChange={(next) => void loadFinals(next)} onPageSizeChange={(next) => { setFinalPageSize(next); void loadFinals(1, next); }} />
    </Panel>
...
function FinalDetailPanel({ detail, saving, onClose, onConfirm, onVoid }: { detail: FinalDetail; saving: boolean; onClose: () => void; onConfirm: () => void; onVoid: () => void }) {
  ...
  return <Panel><div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft p-4"><div><div className="flex items-center gap-2"><h2 className="font-medium text-ink">结差结算单：{master.finalSettlementNo}</h2><span className={`rounded px-2 py-1 text-xs ${statusClass(master.status)}`}>{text(master.status)}</span></div><p className="mt-1 text-sm text-ink-3">{text(master.countryCode)} · {dateValue(master.periodStart)} 至 {dateValue(master.periodEnd)} · {text(master.currency)}</p></div><div className="flex flex-wrap gap-2">{master.status === DRAFT && <Button tone="danger" disabled={saving} onClick={onVoid}><XCircle size={15} />作废草稿</Button>}{master.status === DRAFT && <Button tone="success" disabled={saving} onClick={onConfirm}><CheckCircle2 size={15} />确认结算单</Button>}<Button onClick={onClose}>关闭</Button></div></div><div className="grid gap-px border-b border-line-soft bg-line-soft sm:grid-cols-4"><Metric label="来源单数" value={numberValue(master.sourceCount)} /><Metric label="CAPEX结差总额" value={money(master.capexDifferenceTotal)} negative={isNegative(master.capexDifferenceTotal)} /><Metric label="OPEX结差总额" value={money(master.opexDifferenceTotal)} negative={isNegative(master.opexDifferenceTotal)} /><Metric label="结差合计" value={money(master.differenceTotal)} negative={isNegative(master.differenceTotal)} /></div><StickyTable className="table-scroll table-viewport overflow-auto" tableKey={`balance-final-detail-${master.finalSettlementNo}`}><table className="w-full min-w-[1260px] border-collapse text-sm"><thead className="bg-canvas"><tr>{["来源结差单号", "名称", "结差类型", "国家", "币种", "期间", "明细数", "CAPEX结差", "OPEX结差", "结差合计"].map((label) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={label}>{label}</th>)}</tr></thead><tbody>{detail.sources.map((source) => <tr key={text(source.id)}><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-primary">{text(source.sourceSettlementNo)}</td><td className="max-w-[240px] truncate border-b border-r border-line-soft px-3 py-3">{text(source.sourceTitle) || "-"}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(source.sourceItemTypes) || "-"}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(source.countryCode)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{text(source.currency)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{dateValue(source.periodStart)} 至 {dateValue(source.periodEnd)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{numberValue(source.itemCount)}</td>{["capexDifferenceTotal", "opexDifferenceTotal", "differenceTotal"].map((key) => <td className={`border-b border-r border-line-soft px-3 py-3 text-right ${isNegative(source[key]) ? "text-danger" : ""}`} key={key}>{money(source[key])}</td>)}</tr>)}</tbody></table></StickyTable></Panel>;
}

function Metric({ label, value, negative = false }: { label: string; value: string | number; negative?: boolean }) {
  return <div className="bg-white px-4 py-3"><div className="text-xs text-ink-3">{label}</div><div className={`mt-1 text-base font-medium ${negative ? "text-danger" : "text-ink"}`}>{value}</div></div>;
}
