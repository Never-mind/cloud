    ...
    <Panel>
      ...
      <div className="grid min-w-0 gap-5 p-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
        <div className="min-w-0">
          <div className="mb-3 grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2"><Input className="w-full min-w-0" placeholder="用户名称" value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} /><Input className="w-full min-w-0" placeholder="账号邮箱" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /><Input className="w-full min-w-0" placeholder="初始密码" type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /><select aria-label="新用户角色" className="h-9 w-full min-w-0 rounded border border-line px-3 text-sm" value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value as ManagedUser["role"] })}><option value="user">普通用户</option><option value="admin">管理员</option></select><Button className="justify-self-start" disabled={saving} onClick={() => void createUser()} tone="primary"><UserPlus size={15} />新增</Button></div>
          <div className="overflow-auto border border-line-soft">
            <table className="min-w-full text-sm"><thead className="bg-canvas"><tr>{["名称", "账号", "角色", "状态"].map((label) => <th className="border-b border-r border-line-soft px-3 py-2 text-left font-medium" key={label}>{label}</th>)}</tr></thead><tbody>
