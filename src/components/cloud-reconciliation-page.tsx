...
const columns: Array<[string, string]> = [
  ...
  ["createdAt", "创建时间"], ["updatedAt", "更新时间"], ["confirmedAt", "确认时间"],
];
...
export function CloudReconciliationPage() {
  ...
  async function load(options?: { query?: QueryState; targetTab?: Tab; targetPage?: number }) {
    ...
    try {
      if (targetTab === "mapping") {
        ...
        const data = await requestJson<{ items: Row[]; total: number }>(`/api/cloud/mappings?${params}`);
        setMappings(data.items); setTotal(Number(data.total ?? 0));
        ...
        const params = new URLSearchParams({ keyword, period, page: String(targetPage), pageSize: String(pageSize) });
        appendQueryParams(params, targetQuery);
        const data = await requestJson<{ items: Row[]; total: number; currencyTotals?: Row[] }>(`/api/cloud/rows?${params}`);
        setRows(data.items); setTotal(data.total); setCurrencyTotals(Array.isArray(data.currencyTotals) ? data.currencyTotals : []);
      }
  ...
  async function attachRow(row: Row, file: File) {
    ...
    try {
      ...
      setNotice(`附件“${file.name}”已关联`); setAttachmentVersion((value) => value + 1);
    } catch (error) { setNotice(error instanceof Error ? error.message : "附件上传失败"); }
  }

  /** 供应商付款发票附件：上传到 supplier_payment 名下，与客户开票附件同一套附件表。 */
  async function attachSupplierPaymentInvoice(row: Row, file: File) {
    const form = new FormData(); form.set("file", file);
    try {
      const response = await fetch(`/api/cloud/attachments/supplier_payment/${encodeURIComponent(String(row.id ?? ""))}`, { method: "POST", body: form });
      const data = await response.json(); if (!response.ok) throw new Error(String(data.error ?? "附件上传失败"));
      setNotice(`供应商发票“${file.name}”已关联`); setAttachmentVersion((value) => value + 1);
    } catch (error) { setNotice(error instanceof Error ? error.message : "附件上传失败"); }
  }
  ...
  async function saveSupplierPayment() {
    ...
    const patch = Object.fromEntries(Object.entries(supplierPaymentForm).filter(([key]) => key.startsWith("payment") || key.startsWith("invoice") || ["payerUnitId", "payerUnitName", "currency", "paid", "invoiceStatus", "receivableDate"].includes(key)));
  ...
  return <div className="flex h-[calc(100vh-40px)] flex-col gap-4">
    ...
    <Panel className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-line-soft p-3">{tabs.map(([key, label]) => <button className={`border-b-2 px-3 py-2 text-sm ${tab === key ? "border-primary text-primary" : "border-transparent text-ink-2"}`} key={key} type="button" onClick={() => { setTab(key); setPage(1); }}>{label}</button>)}{/* items-end：这一排里「默认付款单位」是标签在上+输入框在下两行高，底部对齐才不会显得错位 */}
        <div className="ml-auto flex flex-wrap items-end gap-2">{tab === "supplier-payments" ? <div className="w-[260px]"><PartnerSelect kind="undertakingUnits" label="默认付款单位" idValue={defaultPayerUnit?.id} nameValue={defaultPayerUnit?.name} masters={masters} onChange={(selected) => void saveDefaultPayerUnit(selected.id)} /></div> : null}<Input placeholder="搜索客户、账号、批次或供应商" value={keyword} onChange={(event) => setKeyword(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { setPage(1); void load({ targetPage: 1 }); } }} /><Input className="min-w-[130px]" placeholder="账期 YYYY-MM" value={period} onChange={(event) => setPeriod(event.target.value)} /><Button tone="secondary" onClick={() => { setPage(1); void load({ targetPage: 1 }); }}><Search size={15} />查询</Button>{tab === "mapping" ? <Button tone="primary" onClick={() => setMappingForm({ accounts: "", calculationLogic: "catalog" })}><Plus size={15} />新增服务映射</Button> : null}</div></div>
      <div className="grid min-h-0 flex-1 grid-rows-1">{tab === "mapping" ? <MappingTable rows={mappings} onEdit={setMappingForm} query={activeQuery} onQueryChange={updateQuery} loadOptions={(field, optionKeyword) => columnOptions("/api/cloud/mappings", field, optionKeyword)} /> : tab === "supplier-payments" ? <PaymentTable attachmentVersion={attachmentVersion} rows={payments} onAttach={attachSupplierPaymentInvoice} onUpdate={(row) => setSupplierPaymentForm(row)} onTogglePaid={(row) => void updatePayment(row, { paid: row.paid ? 0 : 1 })} onToggleInvoice={(row) => void updatePayment(row, { invoiceStatus: row.invoiceStatus === "issued" ? "not_issued" : "issued" })} query={activeQuery} onQueryChange={updateQuery} loadOptions={(field, optionKeyword) => columnOptions("/api/cloud/supplier-payments", field, optionKeyword)} /> : <RowTable attachmentVersion={attachmentVersion} currencyTotals={currencyTotals} rows={rows} onAttach={attachRow} onConfirm={confirmRow} onDelete={deleteRow} onToggleCollection={toggleCollection} onToggleInvoice={toggleInvoice} onEditRow={setRowForm} onEditCollection={(row) => setCollectionForm(amountFormDefaults(row, "collection"))} onEditInvoice={(row) => setInvoiceForm(amountFormDefaults(row, "invoice"))} query={activeQuery} onQueryChange={updateQuery} loadOptions={(field, optionKeyword) => columnOptions("/api/cloud/rows", field, optionKeyword)} />}</div>
      <PaginationBar page={page} pageSize={pageSize} total={total} onPageChange={setPage} onPageSizeChange={(next) => { setPageSize(next); setPage(1); }} />
    </Panel>
...
function money(value: unknown, currency = "USD") {
  if (value === null || value === undefined || value === "") return "-";
  const formatted = Number.isFinite(Number(value)) ? formatMoneyValue(value) : String(value);
...
function CloudAmountCell({ row, payerKey, payeeKey, payerFallback, payeeFallback, currencyKey, netKey, taxRateKey, taxKey, totalKey, exchangeRateKey, dateKey, parties, action }: { row: Row; payerKey: string; payeeKey: string; payerFallback: string; payeeFallback: string; currencyKey: string; netKey: string; taxRateKey: string; taxKey: string; totalKey: string; exchangeRateKey?: string; dateKey?: string; parties?: Array<[string, string]>; action?: React.ReactNode }) {
  return <div className="min-w-[210px] space-y-0.5 py-1 text-xs leading-5"><div className="truncate text-ink-3" title={flow(row, payerKey, payeeKey, payerFallback, payeeFallback)}>{flow(row, payerKey, payeeKey, payerFallback, payeeFallback)}</div>{parties?.map(([label, key]) => row[key] ? <div className="truncate text-ink-3" key={key}>{label}：{String(row[key])}</div> : null)}{dateKey && row[dateKey] ? <div className="text-ink-3">应收日期 <span className="text-ink-2">{dateOnlyText(row[dateKey])}</span></div> : null}<div className="font-medium text-ink">{money(row[currencyKey] || "USD", "")}</div>{exchangeRateKey && row[exchangeRateKey] ? <div className="text-ink-3">汇率 <span className="text-ink-2">{exchangeRateText(row[exchangeRateKey])}</span></div> : null}<div className="text-ink-3">未税 <span className="text-ink-2">{money(row[netKey], "")}</span></div><div className="text-ink-3">{rateText(row[taxRateKey])}</div><div className="text-ink-3">税金 <span className="text-ink-2">{money(row[taxKey], "")}</span></div><div className="text-ink-2">含税 <span className="font-medium text-ink">{money(row[totalKey], "")}</span></div>{action ? <div className="mt-1 flex items-center gap-1 border-t border-canvas-deep pt-1">{action}</div> : null}</div>;
}

function RowTable({ attachmentVersion, currencyTotals, rows, onAttach, onConfirm, onDelete, onToggleCollection, onToggleInvoice, onEditRow, onEditCollection, onEditInvoice, query, onQueryChange, loadOptions }: { rows: Row[]; onAttach: (row: Row, file: File) => void; onConfirm: (row: Row) => void; onDelete: (row: Row) => void; onToggleCollection: (row: Row) => void; onToggleInvoice: (row: Row) => void; onEditRow: (row: Row) => void; onEditCollection: (row: Row) => void; onEditInvoice: (row: Row) => void; query: QueryState; onQueryChange: (patch: Partial<QueryState>) => void; loadOptions: (field: string, keyword: string) => Promise<TableFilterOption[]>; currencyTotals?: Row[]; attachmentVersion?: number }) {
  ...
  return <StickyTable className="table-scroll h-full w-full overflow-auto" tableKey="cloud-rows"><table className="w-full min-w-[3300px] border-collapse text-sm"><thead className="bg-canvas"><tr>{columns.map(([field, label]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={field}><HeaderMenu field={field} label={label} query={query} onQueryChange={onQueryChange} loadOptions={loadOptions} /></th>)}<th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{rows.map((row) => <tr className="hover:bg-surface-2" key={String(row.id)}>{columns.map(([key]) => <td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 align-top" key={key}>{cell(row, key)}</td>)}<td className="border-b border-line-soft px-3 py-2 align-top"><div className="flex items-center gap-1"><button aria-label="修改华为云对账单" className="inline-flex h-8 w-8 items-center justify-center rounded border border-line bg-white text-ink-2 hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-50" disabled={Boolean(row.confirmed)} title="修改华为云对账单" type="button" onClick={() => onEditRow(row)}><Pencil size={14} /></button><button aria-label={row.confirmed ? "取消确认" : "确认"} className="inline-flex h-8 w-8 items-center justify-center rounded border border-line bg-white text-success hover:border-success disabled:cursor-not-allowed disabled:opacity-50" title={row.confirmed ? "取消确认" : "确认"} type="button" onClick={() => onConfirm(row)}><Check size={14} /></button><button aria-label="删除华为云对账单" className="inline-flex h-8 w-8 items-center justify-center rounded border border-danger-border-strong bg-white text-danger hover:bg-danger-soft" title="删除华为云对账单" type="button" onClick={() => onDelete(row)}><Trash2 size={14} /></button></div></td></tr>)}{!rows.length ? <tr><td className="py-12 text-center text-ink-3" colSpan={columns.length + 1}><EmptyState title="暂无数据" /></td></tr> : null}</tbody>{currencyTotals?.length ? <tfoot>{currencyTotals.map((totals: Row) => <tr key={String(totals.currency)}>{columns.map(([key], index) => <td className="sticky bottom-0 whitespace-nowrap border-t-2 border-line bg-canvas-deep px-3 py-3 align-top font-medium text-ink" key={key}>{index === 0 ? `合计（${display(totals.currency)}）` : totals[key] === undefined || totals[key] === null ? "" : money(totals[key], "")}</td>)}<td className="sticky bottom-0 border-t-2 border-line bg-canvas-deep px-3 py-3" /></tr>)}</tfoot> : null}</table></StickyTable>;
}

function MappingTable({ rows, onEdit, query, onQueryChange, loadOptions }: { rows: Row[]; onEdit: (row: Row) => void; query: QueryState; onQueryChange: (patch: Partial<QueryState>) => void; loadOptions: (field: string, keyword: string) => Promise<TableFilterOption[]> }) {
  return <StickyTable className="table-scroll h-full w-full overflow-auto" tableKey="cloud-mappings"><table className="w-full min-w-[1100px] border-collapse text-sm"><thead className="bg-canvas"><tr>{mappingColumns.map(([field, label]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={field}><HeaderMenu field={field} label={label} query={query} onQueryChange={onQueryChange} loadOptions={loadOptions} /></th>)}<th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}>{mappingColumns.map(([field]) => <td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3" key={field}>{display(row[field])}</td>)}<td className="border-b border-line-soft px-3 py-3"><Button onClick={() => onEdit(row)}><Pencil size={14} />修改</Button></td></tr>)}{!rows.length ? <tr><td className="py-12 text-center text-ink-3" colSpan={mappingColumns.length + 1}><EmptyState title="暂无映射" /></td></tr> : null}</tbody></table></StickyTable>;
}

function PaymentTable({ attachmentVersion, rows, onAttach, onUpdate, onTogglePaid, onToggleInvoice, query, onQueryChange, loadOptions }: { rows: Row[]; onAttach: (row: Row, file: File) => void; onUpdate: (row: Row) => void; onTogglePaid: (row: Row) => void; onToggleInvoice: (row: Row) => void; query: QueryState; onQueryChange: (patch: Partial<QueryState>) => void; loadOptions: (field: string, keyword: string) => Promise<TableFilterOption[]>; attachmentVersion?: number }) {
  const [expanded, setExpanded] = useState<Record<string, boolean>>({});
  function cell(row: Row, key: string) {
    if (key === "period") {
      const id = String(row.id);
      return <div className="flex items-center gap-2"><button aria-label={expanded[id] ? "收起账号明细" : "展开账号明细"} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded border border-line bg-white text-ink-2 hover:border-primary hover:text-primary" title={expanded[id] ? "收起账号明细" : "展开账号明细"} type="button" onClick={() => setExpanded((current) => ({ ...current, [id]: !current[id] }))}>{expanded[id] ? <ChevronDown size={15} /> : <ChevronRight size={15} />}</button><span>{periodText(row[key])}</span></div>;
    }
    ...
    if (key === "invoiceStatus") return <StatusSwitch checked={row.invoiceStatus === "issued"} onClick={() => onToggleInvoice(row)} onLabel="已开票" offLabel="未开票" title="点击切换开票状态" />;
     if (key === "updatedAt") return formatDisplayValue(row[key] as never, "datetime");
     if (["paymentDate", "receivableDate", "invoiceDate"].includes(key)) return dateOnlyText(row[key]);
    return display(row[key]);
  }
  return <StickyTable className="table-scroll h-full w-full overflow-auto" tableKey="cloud-payments"><table className="w-full min-w-[1800px] border-collapse text-sm"><thead className="bg-canvas"><tr>{paymentColumns.map(([field, label]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={field}><HeaderMenu field={field} label={label} query={query} onQueryChange={onQueryChange} loadOptions={loadOptions} /></th>)}<th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{rows.map((row) => { const children = Array.isArray(row.children) ? row.children as Row[] : []; return <Fragment key={String(row.id)}><tr className="hover:bg-surface-2">{paymentColumns.map(([key]) => <td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 align-top" key={key}>{cell(row, key)}</td>)}<td className="border-b border-line-soft px-3 py-2 align-top"><div className="flex items-center gap-1"><button aria-label="编辑供应商实付和开票" className="inline-flex h-8 w-8 items-center justify-center rounded border border-line bg-white text-ink-2 hover:border-primary hover:text-primary" title="编辑供应商实付和开票" type="button" onClick={() => onUpdate(row)}><Pencil size={14} /></button><label aria-label="上传供应商付款发票" className="inline-flex h-8 w-8 cursor-pointer items-center justify-center rounded border border-line bg-white text-ink-2 hover:border-primary hover:text-primary" title="上传供应商付款发票"><FileUp size={14} /><input className="hidden" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) onAttach(row, file); event.target.value = ""; }} /></label><CloudAttachments ownerId={String(row.id ?? "")} ownerType="supplier_payment" refreshToken={attachmentVersion} /></div></td></tr>{expanded[String(row.id)] ? <tr><td className="border-b border-line-soft bg-surface-2 px-10 py-3" colSpan={paymentColumns.length + 1}><table className="w-full min-w-[1000px] border-collapse text-sm"><thead><tr>{["客户", "华为云账号", "供应商应付未税", "税金", "供应商应付含税", "创建时间", "更新时间"].map((label) => <th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium text-ink-2" key={label}>{label}</th>)}</tr></thead><tbody>{children.map((child) => <tr key={String(child.id)}><td className="border-b border-line-soft px-3 py-2">{display(child.customer)}</td><td className="border-b border-line-soft px-3 py-2 text-ink-2">{display(child.account)}</td><td className="border-b border-line-soft px-3 py-2">{money(child.supplierPayableNetAmount, String(child.supplierPayableCurrency || "USD"))}</td><td className="border-b border-line-soft px-3 py-2">{money(child.supplierTaxAmount, String(child.supplierPayableCurrency || "USD"))}</td><td className="border-b border-line-soft px-3 py-2">{money(child.supplierPayableTotalAmount, String(child.supplierPayableCurrency || "USD"))}</td><td className="whitespace-nowrap border-b border-line-soft px-3 py-2">{formatDisplayValue(child.createdAt as never, "datetime")}</td><td className="whitespace-nowrap border-b border-line-soft px-3 py-2">{formatDisplayValue(child.updatedAt as never, "datetime")}</td></tr>)}</tbody></table></td></tr> : null}</Fragment>; })}{!rows.length ? <tr><td className="py-12 text-center text-ink-3" colSpan={paymentColumns.length + 1}><EmptyState title="暂无供应商付款" /></td></tr> : null}</tbody></table></StickyTable>;
}
...
function StatusSwitch({ checked, onClick, onLabel, offLabel, title }: { checked: boolean; onClick: () => void; onLabel: string; offLabel: string; title: string }) {
  return <button aria-checked={checked} className="inline-flex items-center gap-1 text-xs text-ink-2" role="switch" title={title} type="button" onClick={onClick}><span className={`relative inline-flex h-4 w-7 rounded-full ${checked ? "bg-success" : "bg-ink-4"}`}><span className={`absolute top-0.5 h-3 w-3 rounded-full bg-white shadow transition-transform ${checked ? "translate-x-[15px]" : "translate-x-0.5"}`} /></span><span>{checked ? onLabel : offLabel}</span></button>;
}
...
function SupplierPaymentForm({ value, masters, onChange, onCancel, onSave }: { value: Row; masters: MasterSet; onChange: (value: Row) => void; onCancel: () => void; onSave: () => void }) {
  const paymentFields: Array<[string, string, string?]> = [
    ...
    ["paymentTaxRate", "实付税率", "number"], ["paymentTaxAmount", "实付税金", "number"], ["paymentTotalAmount", "实付含税金额", "number"], ["paymentDate", "实付日期", "date"], ["receivableDate", "应收日期", "date"],
  ];
  ...
  return <Modal description="供应商付款按账期和供应商汇总，明细账号只展示供应商应付金额" footer={<><Button onClick={onCancel}>取消</Button><Button onClick={onSave} tone="primary">保存</Button></>} onClose={onCancel} title="编辑供应商实付和开票" widthClass="max-w-4xl"><div className="grid gap-3 border-b border-line-soft pb-4 sm:grid-cols-2"><PartnerSelect kind="undertakingUnits" label="付款单位" idValue={value.payerUnitId} nameValue={value.payerUnitName} masters={masters} onChange={(selected) => onChange({ ...value, payerUnitId: selected.id, payerUnitName: selected.name })} /></div><section className="mt-4"><h3 className="mb-3 text-sm font-medium text-ink">供应商实付</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{paymentFields.map(([key, label, type]) => <label className="space-y-1 text-sm text-ink-2" key={key}><span>{label}</span><Input className="w-full" type={type ?? "text"} value={type === "date" ? dateInputValue(value[key]) : String(value[key] ?? "")} onChange={(event) => onChange(changeField(key, type, event.target.value))} /></label>)}</div><div className="mt-3"><StatusSwitch checked={Boolean(value.paid)} onClick={() => onChange({ ...value, paid: value.paid ? 0 : 1 })} onLabel="已付款" offLabel="未付款" title="点击切换付款状态" /></div></section><section className="mt-5 border-t border-line-soft pt-4"><h3 className="mb-3 text-sm font-medium text-ink">供应商开票</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{invoiceFields.map(([key, label, type]) => <label className="space-y-1 text-sm text-ink-2" key={key}><span>{label}</span><Input className="w-full" type={type ?? "text"} value={type === "date" ? dateInputValue(value[key]) : String(value[key] ?? "")} onChange={(event) => onChange(changeField(key, type, event.target.value))} /></label>)}</div><div className="mt-3"><StatusSwitch checked={value.invoiceStatus === "issued"} onClick={() => onChange({ ...value, invoiceStatus: value.invoiceStatus === "issued" ? "not_issued" : "issued" })} onLabel="已开票" offLabel="未开票" title="点击切换开票状态" /></div></section></Modal>;
}
