...
function MappingForm({ value, masters, onChange, onCancel, onSave }: { value: Row; masters: { suppliers: Master[]; undertakingUnits: Master[]; customers: Master[] }; onChange: (value: Row) => void; onCancel: () => void; onSave: () => void }) {
  const field = (key: string, label: string, options?: Master[]) => <label className="space-y-1 text-sm text-ink-2"><span>{label}</span>{options ? <select className="h-9 w-full rounded border border-line bg-white px-2" value={String(value[key] ?? "")} onChange={(event) => { const selected = options.find((item) => item.id === event.target.value); onChange({ ...value, [key]: event.target.value, [`${key.replace("Id", "Name")}`]: selected?.name ?? "" }); }}><option value="">请选择</option>{options.map((item) => <option key={item.id} value={item.id}>{item.code ? `${item.code} - ` : ""}{item.name}</option>)}</select> : <Input className="w-full" value={String(value[key] ?? "")} onChange={(event) => onChange({ ...value, [key]: event.target.value })} />}</label>;
