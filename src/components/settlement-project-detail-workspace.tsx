...
function InvoiceSection({ invoices, allInvoices, draft, editing, partners, busy, readOnly, attachmentDescriptions, onAttachmentDescriptionChange, onUpload, onDeleteAttachment, state, onState, onPage, onPageSize, onDraftChange, onEditingChange, onEdit, onToggle, onAdd, onSave, onDelete }: InvoiceSectionProps) {
  ...
  const renderColumnHeader = (column: Column) => <th className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium" key={column.key}><ColumnHeader tableKey="invoices" column={column} rows={allInvoices as unknown as TableRow[]} state={state} onState={onState} /></th>;
  ...
  return <Panel>
    ...
    <TableScroll tableKey="settlement-invoices">
      <table className="min-w-[2500px] border-collapse text-sm">
        <thead className="bg-[#f5f7fa]"><tr>{invoiceDataColumns.map(renderColumnHeader)}<th className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium">发票附件</th>{invoiceAuditColumns.map(renderColumnHeader)}<th className="whitespace-nowrap border-b border-[#ebeef5] px-3 py-3 text-left font-medium">操作</th></tr></thead>
        <tbody>
          {rows.map((invoice) => {
            ...
            return <tr className="hover:bg-[#fafafa]" key={invoice.id}>
              {invoiceDataColumns.map((column) => {
                ...
                return <td className="max-w-[210px] truncate whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3" key={column.key}>
...
function InvoiceFormFields({ draft, partners, onChange }: { draft: Record<string, string | boolean>; partners: InvoicePartnerSet; onChange: (key: string, value: string | boolean) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-b border-[#ebeef5] bg-[#fcfcfd] p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8">
      <Field label="类型"><select className="h-9 w-full rounded border border-[#dcdfe6] bg-white px-2" value={String(draft.type)} onChange={(event) => onChange("type", event.target.value)}><option value="cost">成本</option><option value="income">收入</option></select></Field>
      ...
      <Field label="发票币种"><select className="h-9 w-full rounded border border-[#dcdfe6] bg-white px-2" value={String(draft.currency)} onChange={(event) => onChange("currency", event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select></Field>
