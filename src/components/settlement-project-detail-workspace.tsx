...
function InvoiceFormFields({ draft, partners, onChange }: { draft: Record<string, string | boolean>; partners: InvoicePartnerSet; onChange: (key: string, value: string | boolean) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-b border-line-soft bg-surface-2 p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8">
      ...
      <Field label="发票总额"><Input className="w-full" type="number" value={String(draft.invoiceTotal)} onChange={(event) => onChange("invoiceTotal", event.target.value)} /></Field>
      <Field label="不含税总额"><Input className="w-full" readOnly value={money(excluded)} /></Field>
      <Field label="税率(%)"><Input className="w-full" type="number" value={String(draft.taxRate)} onChange={(event) => onChange("taxRate", event.target.value)} /></Field>
      ...
      <Field label="发票汇率"><Input className="w-full" type="number" value={String(draft.exchangeRate)} onChange={(event) => onChange("exchangeRate", event.target.value)} /></Field>
