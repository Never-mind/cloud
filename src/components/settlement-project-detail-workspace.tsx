...
import { ArrowLeft, Check, Download, FilePlus2, FileUp, Paperclip, Save, Trash2 } from "lucide-react";
...
export function SettlementProjectDetailWorkspace() {
  ...
  function applyDetail(next: Detail) {
    ...
    setEditingExpense(Object.fromEntries(next.expenses.map((item) => [item.id, draftExpense(item)])));
    setEditingSale(Object.fromEntries(next.sales.map((item) => [item.id, draftSale(item)])));
  ...
  const unpurchasedPage = pageInfo(visible.unpurchased, pages.unpurchased);
  const purchasedPage = pageInfo(visible.purchased, pages.purchased);
  const expensesPage = pageInfo(visible.expenses, pages.expenses);
  const salesPage = pageInfo(visible.sales, pages.sales);
  const invoicesPage = pageInfo(visible.invoices, pages.invoices);
  const attachmentsPage = pageInfo(visible.attachments, pages.attachments);
...
type PagedRows = { rows: TableRow[]; page: number; total: number; totalPages: number };
type DetailSectionsProps = { detail: Detail; itemDrafts: Record<string, Record<string, string>>; selected: string[]; busy: boolean; importingItems: boolean; expenseDraft: Record<string, string>; saleDraft: Record<string, string>; editingExpense: Record<string, Record<string, string>>; editingSale: Record<string, Record<string, string>>; unpurchased: PagedRows; purchased: PagedRows; expenses: PagedRows; sales: PagedRows; states: Record<TableKey, TableState>; onState: (key: TableKey, patch: Partial<TableState>) => void; onPage: (key: TableKey, page: number) => void; onSelect: (ids: string[]) => void; onItemChange: (id: string, key: string, value: string) => void; onOrder: () => void; onImportItems: (file: File) => void; onSaveItem: (id: string) => void; onReturn: (id: string) => void; onExpenseChange: (id: string, key: string, value: string) => void; onSaleChange: (id: string, key: string, value: string) => void; onSaveExpense: (id: string) => void; onDeleteExpense: (id: string) => void; onSaveSale: (id: string) => void; onDeleteSale: (id: string) => void; onExpenseDraftChange: (key: string, value: string) => void; onSaleDraftChange: (key: string, value: string) => void; onAddExpense: () => void; onAddSale: () => void };

function DetailSections(props: DetailSectionsProps) {
  const { detail } = props;
  return <div className="space-y-5"><PurchaseTable title="未采购商品" tableKey="unpurchased" rows={props.unpurchased} selected={props.selected} itemDrafts={props.itemDrafts} busy={props.busy} importingItems={props.importingItems} project={detail.project} selectable onSelect={props.onSelect} onChange={props.onItemChange} onOrder={props.onOrder} onImportItems={props.onImportItems} state={props.states.unpurchased} onState={(patch) => props.onState("unpurchased", patch)} onPage={(page) => props.onPage("unpurchased", page)} /><PurchaseTable title="已采购商品" tableKey="purchased" rows={props.purchased} selected={props.selected} itemDrafts={props.itemDrafts} busy={props.busy} importingItems={false} project={detail.project} onChange={props.onItemChange} onSave={props.onSaveItem} onReturn={props.onReturn} state={props.states.purchased} onState={(patch) => props.onState("purchased", patch)} onPage={(page) => props.onPage("purchased", page)} /><EntrySection title="其他成本费用" tableKey="expenses" entries={props.expenses} draft={props.expenseDraft} editing={props.editingExpense} kind="expense" project={detail.project} state={props.states.expenses} onState={(patch) => props.onState("expenses", patch)} onPage={(page) => props.onPage("expenses", page)} onDraftChange={props.onExpenseDraftChange} onEditingChange={props.onExpenseChange} onSave={props.onSaveExpense} onDelete={props.onDeleteExpense} onAdd={props.onAddExpense} /><EntrySection title="销售收入明细" tableKey="sales" entries={props.sales} draft={props.saleDraft} editing={props.editingSale} kind="sale" project={detail.project} state={props.states.sales} onState={(patch) => props.onState("sales", patch)} onPage={(page) => props.onPage("sales", page)} onDraftChange={props.onSaleDraftChange} onEditingChange={props.onSaleChange} onSave={props.onSaveSale} onDelete={props.onDeleteSale} onAdd={props.onAddSale} /></div>;
}

function PurchaseTable({ title, tableKey, rows, selected, itemDrafts, project, busy, importingItems, selectable, onSelect, onChange, onOrder, onImportItems, onSave, onReturn, state, onState, onPage }: { title: string; tableKey: "unpurchased" | "purchased"; rows: PagedRows; selected: string[]; itemDrafts: Record<string, Record<string, string>>; project: Project; busy: boolean; importingItems: boolean; selectable?: boolean; onSelect?: (ids: string[]) => void; onChange: (id: string, key: string, value: string) => void; onOrder?: () => void; onImportItems?: (file: File) => void; onSave?: (id: string) => void; onReturn?: (id: string) => void; state: TableState; onState: (patch: Partial<TableState>) => void; onPage: (page: number) => void }) {
  ...
  const total = itemRows.reduce((sum, item) => sum + purchaseBreakdown(item, project).taxExcludedUsd, 0);
...
function filterOptions(rows: TableRow[], column: Column, keyword: string): TableFilterOption[] {
...
function pageInfo(rows: TableRow[], requestedPage: number): PagedRows { const total = rows.length; const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE)); const page = Math.min(Math.max(1, requestedPage), totalPages); return { rows: rows.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE), page, total, totalPages }; }
...
function purchaseBreakdown(item: Item, project: Project) { const amount = Number(item.purchaseQty || 0) * Number(item.purchaseUnitPrice || 0); const rate = Number(item.taxRate || 0) / 100; const included = item.priceType === "tax_included" ? amount : amount * (1 + rate); const excluded = item.priceType === "tax_included" ? amount / (1 + rate || 1) : amount; return { taxIncludedUsd: convertUsd(included, item.currency, project), taxExcludedUsd: convertUsd(excluded, item.currency, project) }; }
function entryBreakdown(entry: Expense | Sale, project: Project) { const rate = Number(entry.taxRate || 0) / 100; const included = entry.priceType === "tax_included" ? Number(entry.amount || 0) : Number(entry.amount || 0) * (1 + rate); const excluded = entry.priceType === "tax_included" ? Number(entry.amount || 0) / (1 + rate || 1) : Number(entry.amount || 0); return { taxIncludedUsd: moneyNumber(convertUsd(included, entry.currency, project)), taxExcludedUsd: moneyNumber(convertUsd(excluded, entry.currency, project)) }; }
...
function Field({ label, children }: { label: string; children: ReactNode }) { return <div className="min-w-0"><span className="mb-1 block text-xs text-[#606266]">{label}</span>{children}</div>; }
