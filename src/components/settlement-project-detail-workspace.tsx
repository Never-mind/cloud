...
type Invoice = { id: string; type: string; accountPeriod: string | null; accountingDate: string | null; companyEntity: string | null; invoiceEntity: string | null; companyEntityId: string | null; invoiceEntityId: string | null; invoiceEntityType: "supplier" | "customer" | null; invoiceDate: string | null; invoiceNo: string | null; invoiceTotal: number; invoiceTaxExcludedTotal: number; taxRate: number; invoiceTaxAmount: number; currency: string; exchangeRate: number; usdAmount: number; receivableDate: string | null; isPaid: boolean; isInvoiced: boolean; attachments?: Attachment[]; createdAt: string; updatedAt: string };
...
const blankInvoice = { type: "cost", accountPeriod: "", accountingDate: "", companyEntity: "", invoiceEntity: "", companyEntityId: "", invoiceEntityId: "", invoiceEntityType: "", invoiceDate: "", invoiceNo: "", invoiceTotal: "", taxRate: "0", currency: "CNY", exchangeRate: "1", receivableDate: "", isPaid: false, isInvoiced: false };
...
function InvoiceFormFields({ draft, partners, onChange }: { draft: Record<string, string | boolean>; partners: InvoicePartnerSet; onChange: (key: string, value: string | boolean) => void }) {
  ...
  return <div className="grid grid-cols-1 gap-3 border-b border-[#ebeef5] bg-[#fcfcfd] p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8"><Field label="类型"><select className="h-9 w-full rounded border border-[#dcdfe6] bg-white px-2" value={String(draft.type)} onChange={(event) => onChange("type", event.target.value)}><option value="cost">成本</option><option value="income">收入</option></select></Field><Field label="账期"><Input className="w-full" type="date" value={dateInputValue(draft.accountPeriod)} onChange={(event) => onChange("accountPeriod", event.target.value)} /></Field><Field label="财务记账日期"><Input className="w-full" type="date" value={dateInputValue(draft.accountingDate)} onChange={(event) => onChange("accountingDate", event.target.value)} /></Field><Field label="承接单位"><PartnerLookup options={partners.undertakingUnits} selectedId={draft.companyEntityId} selectedName={draft.companyEntity} onChange={(selected) => { onChange("companyEntityId", selected.id); onChange("companyEntity", selected.name); }} /></Field><Field label="供应商/客户"><PartnerLookup options={[...partners.suppliers.map((row) => ({ ...row, partnerType: "supplier" })), ...partners.customers.map((row) => ({ ...row, partnerType: "customer" }))]} selectedId={draft.invoiceEntityId} selectedName={draft.invoiceEntity} onChange={(selected) => { onChange("invoiceEntityId", selected.id); onChange("invoiceEntity", selected.name); onChange("invoiceEntityType", selected.partnerType || ""); }} /></Field><Field label="发票日期"><Input className="w-full" type="date" value={dateInputValue(draft.invoiceDate)} onChange={(event) => onChange("invoiceDate", event.target.value)} /></Field><Field label="应收日期"><Input className="w-full" type="date" value={dateInputValue(draft.receivableDate)} onChange={(event) => onChange("receivableDate", event.target.value)} /></Field><Field label="发票号"><Input className="w-full" value={String(draft.invoiceNo)} onChange={(event) => onChange("invoiceNo", event.target.value)} /></Field><Field label="发票总额"><Input className="w-full" type="number" value={String(draft.invoiceTotal)} onChange={(event) => onChange("invoiceTotal", event.target.value)} /></Field><Field label="不含税总额"><Input className="w-full" readOnly value={money(excluded)} /></Field><Field label="税率(%)"><Input className="w-full" type="number" value={String(draft.taxRate)} onChange={(event) => onChange("taxRate", event.target.value)} /></Field><Field label="发票税金"><Input className="w-full" readOnly value={money(tax)} /></Field><Field label="发票币种"><select className="h-9 w-full rounded border border-[#dcdfe6] bg-white px-2" value={String(draft.currency)} onChange={(event) => onChange("currency", event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select></Field><Field label="发票汇率"><Input className="w-full" type="number" value={String(draft.exchangeRate)} onChange={(event) => onChange("exchangeRate", event.target.value)} /></Field><Field label="是否支付"><Toggle checked={Boolean(draft.isPaid)} onChange={(checked) => onChange("isPaid", checked)} label="支付" /></Field><Field label="是否开票"><Toggle checked={Boolean(draft.isInvoiced)} onChange={(checked) => onChange("isInvoiced", checked)} label="开票" /></Field></div>;
}
...
function editableInvoiceField(key: string) { return ["type", "accountPeriod", "accountingDate", "companyEntity", "invoiceEntity", "invoiceDate", "receivableDate", "invoiceNo", "currency", "isPaid", "isInvoiced", "invoiceTotal", "taxRate", "exchangeRate"].includes(key); }
...
const invoiceDateOnlyFields = new Set(["accountPeriod", "accountingDate", "invoiceDate", "receivableDate"]);
...
function InvoiceInput({ invoice, value, field, partners, onChange }: { invoice: Invoice; value: Record<string, string | boolean>; field: string; partners: InvoicePartnerSet; onChange: (value: string | boolean, patch?: Record<string, string | boolean>) => void }) {
  ...
  if (["accountPeriod", "accountingDate", "invoiceDate", "receivableDate"].includes(field)) return <Input type="date" value={dateInputValue(value[field])} onChange={(event) => onChange(event.target.value)} />;
