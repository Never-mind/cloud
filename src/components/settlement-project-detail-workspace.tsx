...
const blankInvoice = { type: "cost", accountPeriod: "", accountingDate: "", companyEntity: "", invoiceEntity: "", companyEntityId: "", invoiceEntityId: "", invoiceEntityType: "", invoiceDate: "", invoiceNo: "", invoiceTotal: "", taxRate: "0", currency: "CNY", exchangeRate: "1", receivableDate: "", actualReceiptDate: "", isPaid: false, isInvoiced: false };
...
const columns: Record<TableKey, Column[]> = {
  unpurchased: [
    ...
    { key: "createdAt", label: "创建时间", type: "datetime", sortable: true, filterable: true }, { key: "updatedAt", label: "更新时间", type: "datetime", sortable: true, filterable: true },
  ],
  purchased: [
    { key: "productCode", label: "产品编码", sortable: true, filterable: true }, { key: "productName", label: "产品名称", sortable: true, filterable: true }, { key: "brand", label: "品牌", sortable: true, filterable: true },
    { key: "plannedQty", label: "报价数量", type: "number", sortable: true, filterable: true }, { key: "purchaseQty", label: "采购数量", type: "number", sortable: true, filterable: true }, { key: "purchaseUnitPrice", label: "采购单价", type: "money", sortable: true, filterable: true },
  ...
  expenses: [
    ...
    { key: "priceType", label: "价格方式", sortable: true, filterable: true }, { key: "taxRate", label: "税率(%)", type: "money", sortable: true, filterable: true }, { key: "invoiceNo", label: "发票号", sortable: true, filterable: true }, { key: "createdAt", label: "创建时间", type: "datetime", sortable: true, filterable: true }, { key: "updatedAt", label: "更新时间", type: "datetime", sortable: true, filterable: true },
  ],
  sales: [
    ...
    { key: "taxRate", label: "税率(%)", type: "money", sortable: true, filterable: true }, { key: "invoiceNo", label: "发票号", sortable: true, filterable: true }, { key: "receivedAt", label: "收款日期", type: "date", sortable: true, filterable: true }, { key: "createdAt", label: "创建时间", type: "datetime", sortable: true, filterable: true }, { key: "updatedAt", label: "更新时间", type: "datetime", sortable: true, filterable: true },
  ],
  invoices: [
    { key: "type", label: "类型", sortable: true, filterable: true }, { key: "accountPeriod", label: "账期", type: "date", sortable: true, filterable: true }, { key: "accountingDate", label: "财务记账日期", type: "date", sortable: true, filterable: true }, { key: "companyEntity", label: "承接单位", sortable: true, filterable: true }, { key: "invoiceEntity", label: "供应商/客户", sortable: true, filterable: true }, { key: "invoiceDate", label: "发票日期", type: "date", sortable: true, filterable: true }, { key: "receivableDate", label: "应收日期", type: "date", sortable: true, filterable: true }, { key: "actualReceiptDate", label: "实收日期", type: "date", sortable: true, filterable: true }, { key: "invoiceNo", label: "发票号", sortable: true, filterable: true },
    { key: "currency", label: "发票币种", sortable: true, filterable: true }, { key: "isPaid", label: "是否支付", sortable: true, filterable: true }, { key: "isInvoiced", label: "是否开票", sortable: true, filterable: true }, { key: "createdAt", label: "创建时间", type: "datetime", sortable: true, filterable: true }, { key: "updatedAt", label: "更新时间", type: "datetime", sortable: true, filterable: true },
  ],
  attachments: [
    { key: "fileName", label: "文件名", sortable: true, filterable: true }, { key: "fileType", label: "类型", sortable: true, filterable: true }, { key: "fileSize", label: "大小", type: "fileSize", sortable: true, filterable: true }, { key: "description", label: "说明", sortable: true, filterable: true }, { key: "uploadedByName", label: "上传人", sortable: true, filterable: true }, { key: "uploadedAt", label: "上传时间", type: "datetime", sortable: true, filterable: true },
  ],
};
...
const invoiceDataColumns: Column[] = [
  { key: "type", label: "类型", sortable: true, filterable: true },
  { key: "accountPeriod", label: "账期", type: "date", sortable: true, filterable: true },
  { key: "accountingDate", label: "财务记账日期", type: "date", sortable: true, filterable: true },
  ...
  { key: "invoiceTaxExcludedTotal", label: "发票不含税总额", type: "money", sortable: true, filterable: true },
  { key: "taxRate", label: "税率(%)", type: "money", sortable: true, filterable: true },
  { key: "invoiceTaxAmount", label: "发票税金", type: "money", sortable: true, filterable: true },
  { key: "currency", label: "发票币种", sortable: true, filterable: true },
  { key: "exchangeRate", label: "发票汇率", type: "money", sortable: true, filterable: true },
  { key: "isPaid", label: "是否支付", sortable: true, filterable: true },
  { key: "isInvoiced", label: "是否开票", sortable: true, filterable: true },
  { key: "usdAmount", label: "USD金额", type: "money", sortable: true, filterable: true },
];

const invoiceAuditColumns: Column[] = [
  { key: "createdAt", label: "创建时间", type: "datetime", sortable: true, filterable: true },
  { key: "updatedAt", label: "更新时间", type: "datetime", sortable: true, filterable: true },
];
...
export function SettlementProjectDetailWorkspace() {
  ...
  const [invoicePartners, setInvoicePartners] = useState<{ suppliers: Row[]; customers: Row[]; undertakingUnits: Row[] }>({ suppliers: [], customers: [], undertakingUnits: [] });
  ...
  if (!detail) return <Panel className="p-8 text-sm text-ink-3">{error || <LoadingBlock />}</Panel>;

  const project = detail.project;
  const readOnly = project.status === "closed";
  const nextStatus = project.status === "purchasing" ? "procurement_completed" : project.status === "procurement_completed" ? "accepting" : project.status === "accepting" ? "acceptance_completed" : project.status === "acceptance_completed" ? "closed" : "";
  ...
  async function toggleInvoiceStatus(invoiceId: string, field: "isPaid" | "isInvoiced", checked: boolean) {
    const invoice = detail?.invoices.find((item) => item.id === invoiceId);
    if (!invoice) return;
    await write(`/api/po/settlement-projects/${encodeURIComponent(projectId)}/invoices/${encodeURIComponent(invoiceId)}`, "PUT", { ...draftInvoice(invoice), [field]: checked });
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        <button className="inline-flex items-center gap-1 text-sm text-primary hover:underline" type="button" onClick={() => openRoute("/po/settlement-projects", "项目结算")}><ArrowLeft size={15} />返回项目结算</button>
        <span className="text-line">/</span><h1 className="text-xl font-medium text-ink">{project.projectNo}</h1><span className={`rounded px-2 py-1 text-xs ${statusClass(project.status)}`}>{statusLabel(project.status)}</span>
        <div className="ml-auto flex gap-2"><a className="inline-flex h-9 items-center gap-1 rounded border border-line bg-white px-3 text-sm text-ink-2 hover:opacity-85" href={`/api/po/settlement-projects/${encodeURIComponent(projectId)}/export`}><Download size={15} />导出</a>{nextStatus ? <Button tone={nextStatus === "closed" ? "success" : "primary"} disabled={busy || (nextStatus === "procurement_completed" && !canCompleteProcurement)} onClick={() => void changeStatus()}><Check size={15} />{nextStatus === "procurement_completed" ? "采购完成" : nextStatus === "accepting" ? "进入验收" : nextStatus === "acceptance_completed" ? "验收完成" : "完结项目"}</Button> : null}</div>
      </div>
      {error ? <div className="border border-tag-red-border bg-tag-red px-3 py-2 text-sm text-tag-red-fg">{error}<button className="ml-3 underline" type="button" onClick={() => setError("")}>关闭</button></div> : null}
...
function PurchaseTable({ title, tableKey, rows, selected, itemDrafts, editingRows, project, busy, importingItems, selectable, onSelect, onChange, onOrder, onImportItems, onEdit, onSave, onReturn, state, onState, onPage, onPageSize }: PurchaseTableProps) {
  ...
  const locked = project.status === "closed";
  const action = selectable ? <div className="flex flex-wrap justify-end gap-2"><a className="inline-flex h-9 items-center gap-1 rounded border border-line bg-white px-3 text-sm text-ink-2 hover:border-ink-4" href={"/api/po/settlement-projects/" + encodeURIComponent(project.id) + "/items/template"}><Download size={15} />下载模板</a><a className="inline-flex h-9 items-center gap-1 rounded border border-line bg-white px-3 text-sm text-ink-2 hover:border-ink-4" href={"/api/po/settlement-projects/" + encodeURIComponent(project.id) + "/items/export"}><Download size={15} />导出未采购</a><Button disabled={busy || importingItems || locked} onClick={() => importInputRef.current?.click()}><FileUp size={15} />{importingItems ? "导入中..." : "导入未采购"}</Button><input ref={importInputRef} className="hidden" type="file" accept=".xlsx,.xls" onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ""; if (file) onImportItems?.(file); }} /><Button tone="primary" disabled={busy || locked || !selected.length} onClick={onOrder}><Check size={15} />确认采购</Button></div> : undefined;


  return <Panel><SectionTitle title={title} action={action} /><TableScroll tableKey={"settlement-" + tableKey}><table className="min-w-[1900px] border-collapse text-sm"><thead className="bg-canvas"><tr>{selectable ? <th className="whitespace-nowrap table-select-cell border-b border-r border-line-soft py-3 text-center font-medium [&>input]:h-4 [&>input]:w-4 [&>input]:align-middle"><input aria-label="全选" type="checkbox" checked={Boolean(allSelected)} onChange={(event) => { const ids = itemRows.map((item) => item.id); onSelect?.(event.target.checked ? Array.from(new Set([...selected, ...ids])) : selected.filter((id) => !ids.includes(id))); }} /></th> : null}{columns[tableKey].map((column) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={column.key}><ColumnHeader tableKey={tableKey} column={column} rows={rows.rows} state={state} onState={onState} /></th>)}<th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">采购总价</th><th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">不含税采购金额（USD）</th><th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">含税采购金额（USD）</th>{onSave ? <th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th> : null}</tr></thead><tbody>{itemRows.map((item) => { const draft = itemDrafts[item.id] || draftItem(item); const editing = Boolean(editingRows?.[item.id]); const inputMode = Boolean(selectable || editing); const preview = previewPurchaseItem(item, draft, inputMode); const amounts = purchaseBreakdown(preview, project); return <tr className="hover:bg-surface-2" key={item.id}>{selectable ? <td className="table-select-cell border-b border-r border-line-soft py-3 text-center [&>input]:h-4 [&>input]:w-4 [&>input]:align-middle"><input type="checkbox" checked={selected.includes(item.id)} onChange={(event) => onSelect?.(event.target.checked ? [...selected, item.id] : selected.filter((id) => id !== item.id))} /></td> : null}{columns[tableKey].map((column) => <td className="max-w-[230px] truncate whitespace-nowrap border-b border-r border-line-soft px-3 py-3" key={column.key}>{editablePurchaseField(column.key) && inputMode ? <PurchaseInput item={preview} draft={draft} field={column.key} onChange={(value) => onChange(item.id, column.key, value)} /> : formatColumnValue(column.key, item[column.key as keyof Item], column.type)}</td>)}<td className="numeric-cell border-b border-r border-line-soft px-3 py-3">{money(preview.purchaseQty * preview.purchaseUnitPrice)}</td><td className="numeric-cell border-b border-r border-line-soft px-3 py-3">{money(amounts.taxExcludedUsd)}</td><td className="numeric-cell border-b border-r border-line-soft px-3 py-3">{money(amounts.taxIncludedUsd)}</td>{onSave ? <td className="whitespace-nowrap border-b border-line-soft px-3 py-3">{editing ? <Button disabled={busy || locked} onClick={() => onSave(item.id)}><Save size={14} />保存</Button> : <Button disabled={busy || locked} onClick={() => onEdit?.(item.id)}><Pencil size={14} />修改</Button>}<Button className="ml-2" tone="danger" disabled={busy || locked} onClick={() => onReturn?.(item.id)}><ArrowLeft size={14} />退回</Button></td> : null}</tr>; })}{!itemRows.length ? <Empty colSpan={(selectable ? 1 : 0) + columns[tableKey].length + 3 + (onSave ? 1 : 0)} text={selectable ? "暂无未采购商品" : "暂无已采购商品"} /> : null}</tbody>{rows.summaryRows.length ? <tfoot><tr><td className="border-t border-line-soft px-3 py-3 font-medium" colSpan={(selectable ? 1 : 0) + 3}>合计</td><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{formatQuantity(summary.plannedQty)}</td><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{formatQuantity(summary.purchaseQty)}</td><td className="border-t border-r border-line-soft px-3 py-3" colSpan={7} /><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{formatCurrencyTotals(summary.purchaseTotalsByCurrency)}</td><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{money(summary.taxExcludedUsd)}</td><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{money(summary.taxIncludedUsd)}</td>{onSave ? <td className="border-t border-line-soft px-3 py-3" /> : null}</tr></tfoot> : null}</table></TableScroll><PaginationBar page={rows.page} pageSize={rows.pageSize} total={rows.total} onPageChange={onPage} onPageSizeChange={onPageSize} /></Panel>;
}

function EntrySection({ title, tableKey, entries, draft, editing, kind, project, busy, state, onState, onPage, onPageSize, onDraftChange, onEditingChange, onEdit, onSave, onDelete, onAdd }: { title: string; tableKey: "expenses" | "sales"; entries: PagedRows; draft: Record<string, string>; editing: Record<string, Record<string, string>>; kind: "expense" | "sale"; project: Project; busy: boolean; state: TableState; onState: (patch: Partial<TableState>) => void; onPage: (page: number) => void; onPageSize: (pageSize: number) => void; onDraftChange: (key: string, value: string) => void; onEditingChange: (id: string, key: string, value: string) => void; onEdit: (entry: Expense | Sale) => void; onSave: (id: string) => void; onDelete: (id: string) => void; onAdd: () => void }) {
  ...
  const formGrid = isExpense
    ? "grid gap-3 border-b border-line-soft bg-surface-2 p-4 md:grid-cols-3 xl:grid-cols-[minmax(170px,1.35fr)_minmax(150px,1.2fr)_minmax(140px,1.1fr)_minmax(120px,1fr)_max-content_minmax(100px,.75fr)_minmax(130px,1fr)]"
    : "grid gap-3 border-b border-line-soft bg-surface-2 p-4 md:grid-cols-3 xl:grid-cols-[minmax(180px,1.35fr)_minmax(145px,1.1fr)_minmax(130px,1fr)_max-content_minmax(100px,.75fr)_minmax(130px,1fr)]";
  const locked = project.status === "closed";

  return <Panel><SectionTitle title={title} action={<>{locked ? <span className="mr-3 text-xs text-ink-3">项目已完结，数据只读</span> : null}<Button tone="primary" disabled={locked || busy} onClick={onAdd}><FilePlus2 size={15} />新增</Button></>} /><fieldset className="min-w-0" disabled={locked}><div className={formGrid}>{isExpense ? <Field label="费用类型" required><Select className="w-full" value={draft.type} onChange={(event) => onDraftChange("type", event.target.value)}><option value="first_mile_freight">头程运费</option><option value="customs_fee">清关费</option><option value="labor_fee">人力费</option><option value="equipment_service_fee">设备服务费</option><option value="other">其他</option></Select></Field> : null}<Field label={isExpense ? "说明" : "收入说明"}><Input className="w-full" value={draft.description || ""} onChange={(event) => onDraftChange("description", event.target.value)} /></Field><Field label={isExpense ? "金额" : "收入金额"} required><NumberInput className="w-full" value={draft.amount || ""} onChange={(text) => onDraftChange("amount", text)} /></Field><Field label="币种" required><Select className="w-full" value={draft.currency} onChange={(event) => onDraftChange("currency", event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</Select></Field><Field label="价格方式" required><PriceType value={draft.priceType} onChange={(value) => onDraftChange("priceType", value)} /></Field><Field label="税率（%）" required><NumberInput className="w-full" value={draft.taxRate} onChange={(text) => onDraftChange("taxRate", text)} /></Field><Field label={isExpense ? "发票号" : "收款日期"} required={!isExpense}><Input className="w-full" placeholder={isExpense ? "可选" : "请选择日期"} type={isExpense ? "text" : "date"} value={isExpense ? draft.invoiceNo : draft.receivedAt} onChange={(event) => onDraftChange(isExpense ? "invoiceNo" : "receivedAt", event.target.value)} /></Field></div></fieldset><TableScroll tableKey={`settlement-${tableKey}`}><table className="min-w-[1650px] border-collapse text-sm"><thead className="bg-canvas"><tr>{columns[tableKey].map((column) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={column.key}><ColumnHeader tableKey={tableKey} column={column} rows={entries.rows} state={state} onState={onState} /></th>)}<th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">{isExpense ? "不含税成本（USD）" : "不含税销售收入（USD）"}</th><th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">{isExpense ? "含税成本（USD）" : "含税销售收入（USD）"}</th><th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{items.map((entry) => { const isEditing = Boolean(editing[entry.id]); const value = editing[entry.id] || (isExpense ? draftExpense(entry as Expense) : draftSale(entry as Sale)); const preview = isEditing ? { ...entry, ...value, amount: Number(value.amount || 0), taxRate: Number(value.taxRate || 0) } as Expense | Sale : entry; const amounts = entryBreakdown(preview, project); return <tr className="hover:bg-surface-2" key={entry.id}>{columns[tableKey].map((column) => <td className="max-w-[220px] truncate whitespace-nowrap border-b border-r border-line-soft px-3 py-3" key={column.key}>{editableEntryField(column.key) && isEditing ? <EntryInput entry={preview} value={value} field={column.key} kind={kind} onChange={(next) => onEditingChange(entry.id, column.key, next)} /> : formatColumnValue(column.key, entry[column.key as keyof typeof entry], column.type)}</td>)}<td className="numeric-cell border-b border-r border-line-soft px-3 py-3">{money(amounts.taxExcludedUsd)}</td><td className="numeric-cell border-b border-r border-line-soft px-3 py-3">{money(amounts.taxIncludedUsd)}</td><td className="whitespace-nowrap border-b border-line-soft px-3 py-3">{isEditing ? <Button disabled={locked || busy} onClick={() => onSave(entry.id)}><Save size={14} />保存</Button> : <Button disabled={locked || busy} onClick={() => onEdit(entry)}><Pencil size={14} />修改</Button>}<Button className="ml-2" tone="danger" disabled={locked || busy} onClick={() => onDelete(entry.id)}><Trash2 size={14} />删除</Button></td></tr>; })}{!items.length ? <Empty colSpan={columns[tableKey].length + 3} text={isExpense ? "暂无成本费用" : "暂无销售收入明细"} /> : null}</tbody>{entries.summaryRows.length ? <tfoot><tr><td className="border-t border-line-soft px-3 py-3 font-medium" colSpan={2}>合计</td><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{formatCurrencyTotals(summary.amountTotalsByCurrency)}</td><td className="border-t border-r border-line-soft px-3 py-3" colSpan={6} /><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{money(summary.taxExcludedUsd)}</td><td className="numeric-cell border-t border-r border-line-soft px-3 py-3">{money(summary.taxIncludedUsd)}</td><td className="border-t border-line-soft px-3 py-3" /></tr></tfoot> : null}</table></TableScroll><PaginationBar page={entries.page} pageSize={entries.pageSize} total={entries.total} onPageChange={onPage} onPageSizeChange={onPageSize} /></Panel>;
}

type InvoicePartnerSet = { suppliers: Row[]; customers: Row[]; undertakingUnits: Row[] };
...
type InvoiceSectionProps = {
  ...
  editing: Record<string, Record<string, string | boolean>>;
  partners: InvoicePartnerSet;
  busy: boolean;
  readOnly: boolean;
  attachmentDescriptions: Record<string, string>;
  onAttachmentDescriptionChange: (invoiceId: string, value: string) => void;
  onUpload: (invoiceId: string, event: ChangeEvent<HTMLInputElement>) => void;
  ...
  onEditingChange: (id: string, key: string, value: string | boolean, patch?: Record<string, string | boolean>) => void;
  onEdit: (invoice: Invoice) => void;
  onToggle: (id: string, field: "isPaid" | "isInvoiced", checked: boolean) => void;
  onAdd: () => void;
  onSave: (id: string) => void;
  onDelete: (id: string) => void;
};

function InvoiceSection({ invoices, allInvoices, draft, editing, partners, busy, readOnly, attachmentDescriptions, onAttachmentDescriptionChange, onUpload, onDeleteAttachment, state, onState, onPage, onPageSize, onDraftChange, onEditingChange, onEdit, onToggle, onAdd, onSave, onDelete }: InvoiceSectionProps) {
  const rows = invoices.rows as unknown as Invoice[];
  const renderColumnHeader = (column: Column) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={column.key}><ColumnHeader tableKey="invoices" column={column} rows={allInvoices as unknown as TableRow[]} state={state} onState={onState} /></th>;

  return <Panel>
    <SectionTitle title="发票管理" action={<Button disabled={readOnly} tone="primary" onClick={onAdd}><FilePlus2 size={15} />新增发票</Button>} />
    <InvoiceFormFields draft={draft} partners={partners} onChange={onDraftChange} />
    <TableScroll tableKey="settlement-invoices">
      <table className="min-w-[2500px] border-collapse text-sm">
        <thead className="bg-canvas"><tr>{invoiceDataColumns.map(renderColumnHeader)}<th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">发票附件</th>{invoiceAuditColumns.map(renderColumnHeader)}<th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead>
        <tbody>
          {rows.map((invoice) => {
            const isEditing = Boolean(editing[invoice.id]);
            const value = editing[invoice.id] || draftInvoice(invoice);
            const preview = invoiceBreakdown(invoice, value);
            const attachments = invoice.attachments ?? [];
            const onInvoiceChange = (field: string, next: string | boolean, patch?: Record<string, string | boolean>) => onEditingChange(invoice.id, field, next, patch);
            return <tr className="hover:bg-surface-2" key={invoice.id}>
              {invoiceDataColumns.map((column) => {
                const fieldValue = invoiceColumnValue(column.key, invoice, preview);
                const toggleField = isInvoiceToggleField(column.key);
                return <td className="max-w-[210px] truncate whitespace-nowrap border-b border-r border-line-soft px-3 py-3" key={column.key}>
                  {isEditing && editableInvoiceField(column.key) && !toggleField ? <InvoiceInput invoice={invoice} value={value} field={column.key} partners={partners} onChange={(next, patch) => onInvoiceChange(column.key, next, patch)} /> : toggleField ? <Toggle checked={Boolean(isEditing ? value[column.key] : invoice[column.key as "isPaid" | "isInvoiced"])} disabled={readOnly || busy} onChange={(checked) => isEditing ? onInvoiceChange(column.key, checked) : onToggle(invoice.id, column.key as "isPaid" | "isInvoiced", checked)} label={column.key === "isPaid" ? "支付" : "开票"} /> : formatColumnValue(column.key, fieldValue, column.type)}
                </td>;
              })}
              <td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3"><div className="space-y-2">{attachments.map((attachment) => <div className="flex max-w-[280px] items-center gap-2" key={attachment.id}><span className="min-w-0 flex-1 truncate" title={attachment.fileName}>{attachment.fileName}</span><a className="inline-flex h-8 w-8 shrink-0 items-center justify-center text-primary hover:text-info" aria-label="下载附件" title="下载附件" href={`/api/po/settlement-projects/${attachment.projectId}/invoices/${invoice.id}/attachments/${attachment.id}/download`}><Download size={15} /></a>{true ? <button className="inline-flex h-8 w-8 shrink-0 items-center justify-center text-danger hover:text-danger-strong" type="button" aria-label="删除附件" title="删除附件" onClick={() => onDeleteAttachment(attachment.id)}><Trash2 size={15} /></button> : null}</div>)}{!attachments.length ? <div className="flex items-center gap-2"><Input className="min-w-[150px]" placeholder="附件说明" value={attachmentDescriptions[invoice.id] ?? ""} onChange={(event) => onAttachmentDescriptionChange(invoice.id, event.target.value)} /><label className="inline-flex h-9 shrink-0 cursor-pointer items-center gap-1 rounded border border-primary px-3 text-sm text-primary"><Paperclip size={15} />上传<input className="hidden" type="file" onChange={(event) => void onUpload(invoice.id, event)} /></label></div> : null}{null}</div></td>
              {invoiceAuditColumns.map((column) => <td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3" key={column.key}>{formatColumnValue(column.key, invoice[column.key as keyof Invoice], column.type)}</td>)}
              <td className="whitespace-nowrap border-b border-line-soft px-3 py-3">{isEditing ? <Button disabled={busy} tone="primary" onClick={() => onSave(invoice.id)}><Save size={14} />保存</Button> : <Button disabled={readOnly || busy} onClick={() => onEdit(invoice)}><Pencil size={14} />修改</Button>}<Button className="ml-2" tone="danger" disabled={readOnly || busy} onClick={() => onDelete(invoice.id)}><Trash2 size={15} />删除</Button></td>
            </tr>;
          })}
          {!rows.length ? <Empty colSpan={invoiceDataColumns.length + invoiceAuditColumns.length + 2} text="暂无发票" /> : null}
        </tbody>
      </table>
    </TableScroll>
    <PaginationBar page={invoices.page} pageSize={invoices.pageSize} total={invoices.total} onPageChange={onPage} onPageSizeChange={onPageSize} />
  </Panel>;
}

function InvoiceFormFields({ draft, partners, onChange }: { draft: Record<string, string | boolean>; partners: InvoicePartnerSet; onChange: (key: string, value: string | boolean) => void }) {
  const total = Number(draft.invoiceTotal || 0);
  const rate = Number(draft.taxRate || 0) / 100;
  const excluded = total / (1 + rate || 1);
  const tax = total - excluded;
  const invoicePartners = [
    ...partners.suppliers.map((row) => ({ ...row, partnerType: "supplier" })),
    ...partners.customers.map((row) => ({ ...row, partnerType: "customer" })),
  ];

  return (
    <div className="grid grid-cols-1 gap-3 border-b border-line-soft bg-surface-2 p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8">
      <Field label="类型"><Select className="w-full" value={String(draft.type)} onChange={(event) => onChange("type", event.target.value)}><option value="cost">成本</option><option value="income">收入</option></Select></Field>
      <Field label="账期"><Input className="w-full" type="date" value={dateInputValue(draft.accountPeriod)} onChange={(event) => onChange("accountPeriod", event.target.value)} /></Field>
      <Field label="财务记账日期"><Input className="w-full" type="date" value={dateInputValue(draft.accountingDate)} onChange={(event) => onChange("accountingDate", event.target.value)} /></Field>
      <Field label="承接单位"><PartnerLookup options={partners.undertakingUnits} selectedId={draft.companyEntityId} selectedName={draft.companyEntity} onChange={(selected) => { onChange("companyEntityId", selected.id); onChange("companyEntity", selected.name); }} /></Field>
      <Field label="供应商/客户"><PartnerLookup options={invoicePartners} selectedId={draft.invoiceEntityId} selectedName={draft.invoiceEntity} onChange={(selected) => { onChange("invoiceEntityId", selected.id); onChange("invoiceEntity", selected.name); onChange("invoiceEntityType", selected.partnerType || ""); }} /></Field>
      <Field label="发票日期"><Input className="w-full" type="date" value={dateInputValue(draft.invoiceDate)} onChange={(event) => onChange("invoiceDate", event.target.value)} /></Field>
      <Field label="应收日期"><Input className="w-full" type="date" value={dateInputValue(draft.receivableDate)} onChange={(event) => onChange("receivableDate", event.target.value)} /></Field>
      <Field label="实收日期"><Input className="w-full" type="date" value={dateInputValue(draft.actualReceiptDate)} onChange={(event) => onChange("actualReceiptDate", event.target.value)} /></Field>
      <Field label="发票号"><Input className="w-full" value={String(draft.invoiceNo)} onChange={(event) => onChange("invoiceNo", event.target.value)} /></Field>
      <Field label="发票总额"><NumberInput className="w-full" value={String(draft.invoiceTotal)} onChange={(text) => onChange("invoiceTotal", text)} /></Field>
      <Field label="不含税总额"><Input className="w-full" readOnly value={money(excluded)} /></Field>
      <Field label="税率(%)"><NumberInput className="w-full" value={String(draft.taxRate)} onChange={(text) => onChange("taxRate", text)} /></Field>
      <Field label="发票税金"><Input className="w-full" readOnly value={money(tax)} /></Field>
      <Field label="发票币种"><Select className="w-full" value={String(draft.currency)} onChange={(event) => onChange("currency", event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</Select></Field>
      <Field label="发票汇率"><NumberInput className="w-full" value={String(draft.exchangeRate)} onChange={(text) => onChange("exchangeRate", text)} /></Field>
      <Field label="是否支付"><Toggle checked={Boolean(draft.isPaid)} onChange={(checked) => onChange("isPaid", checked)} label="支付" /></Field>
      <Field label="是否开票"><Toggle checked={Boolean(draft.isInvoiced)} onChange={(checked) => onChange("isInvoiced", checked)} label="开票" /></Field>
    </div>
  );
}
...
function AttachmentSection({ attachments, description, readOnly, state, onState, onPage, onPageSize, onDescriptionChange, onUpload, onDelete }: { attachments: PagedRows; description: string; readOnly: boolean; state: TableState; onState: (patch: Partial<TableState>) => void; onPage: (page: number) => void; onPageSize: (pageSize: number) => void; onDescriptionChange: (value: string) => void; onUpload: (event: ChangeEvent<HTMLInputElement>) => void; onDelete: (id: string) => void }) {
  ...
  return <Panel><SectionTitle title="附件管理" action={!readOnly ? <label className="inline-flex h-9 cursor-pointer items-center gap-1 rounded border border-primary bg-primary px-3 text-sm text-white"><Paperclip size={15} />上传附件<input className="hidden" type="file" onChange={onUpload} /></label> : undefined} /><div className="border-b border-line-soft p-4"><Input disabled={readOnly} placeholder="附件说明" value={description} onChange={(event) => onDescriptionChange(event.target.value)} /></div><TableScroll tableKey="settlement-attachments"><table className="min-w-[1200px] border-collapse text-sm"><thead className="bg-canvas"><tr>{columns.attachments.map((column) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={column.key}><ColumnHeader tableKey="attachments" column={column} rows={attachments.rows} state={state} onState={onState} /></th>)}<th className="whitespace-nowrap border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{rows.map((attachment) => <tr key={attachment.id}>{columns.attachments.map((column) => <td className="max-w-[240px] truncate whitespace-nowrap border-b border-r border-line-soft px-3 py-3" key={column.key}>{formatColumnValue(column.key, attachment[column.key as keyof Attachment], column.type)}</td>)}<td className="border-b border-line-soft px-3 py-3"><a className="mr-3 inline-flex items-center gap-1 text-primary" href={`/api/po/settlement-projects/${attachment.projectId}/attachments/${attachment.id}/download`}><Download size={14} />下载</a>{!readOnly ? <button className="inline-flex items-center gap-1 text-danger" type="button" onClick={() => onDelete(attachment.id)}><Trash2 size={14} />删除</button> : null}</td></tr>)}{!rows.length ? <Empty colSpan={columns.attachments.length + 1} text="暂无附件" /> : null}</tbody></table></TableScroll><PaginationBar page={attachments.page} pageSize={attachments.pageSize} total={attachments.total} onPageChange={onPage} onPageSizeChange={onPageSize} /></Panel>;
}
...
function editableInvoiceField(key: string) { return ["type", "accountPeriod", "accountingDate", "companyEntity", "invoiceEntity", "invoiceDate", "receivableDate", "actualReceiptDate", "invoiceNo", "currency", "isPaid", "isInvoiced", "invoiceTotal", "taxRate", "exchangeRate"].includes(key); }
function isInvoiceToggleField(key: string): key is "isPaid" | "isInvoiced" { return key === "isPaid" || key === "isInvoiced"; }
function legacyFormatColumnValue(key: string, value: unknown, type?: string): string { if (value === null || value === undefined || value === "") return "-"; if (key === "type") return String(value) === "income" ? "收入" : String(value) === "cost" ? "成本" : ["first_mile_freight", "customs_fee", "labor_fee", "equipment_service_fee", "other"].includes(String(value)) ? expenseLabel(String(value)) : String(value); if (key === "priceType") return String(value) === "tax_included" ? "含税价" : "未税价"; if (key === "isPaid") return value ? "已支付" : "未支付"; if (key === "isInvoiced") return value ? "已开票" : "未开票"; if (type === "money" || key === "amount" || key === "taxRate") return money(Number(value)); if (type === "number") return Number(value).toLocaleString("en-US"); if (type === "fileSize") return formatFileSize(Number(value)); if (type === "datetime" || key.endsWith("At")) return formatDisplayValue(value as never, "datetime"); if (type === "date" || key.endsWith("Date")) return formatDisplayValue(value as never, "date"); return String(value); }
const invoiceDateOnlyFields = new Set(["accountPeriod", "accountingDate", "invoiceDate", "receivableDate", "actualReceiptDate"]);
function formatColumnValue(key: string, value: unknown, type?: string): string { if (value === null || value === undefined || value === "") return "-"; if (invoiceDateOnlyFields.has(key)) return dateInputValue(value) || "-"; return legacyFormatColumnValue(key, value, type); }
...
function invoiceColumnValue(key: string, invoice: Invoice, preview: ReturnType<typeof invoiceBreakdown>) { if (key === "invoiceTaxExcludedTotal") return preview.invoiceTaxExcludedTotal; if (key === "invoiceTaxAmount") return preview.invoiceTaxAmount; if (key === "usdAmount") return preview.usdAmount; return invoice[key as keyof Invoice]; }
...
function draftInvoice(item: Invoice) { return { type: item.type, accountPeriod: dateInputValue(item.accountPeriod), accountingDate: dateInputValue(item.accountingDate), companyEntity: item.companyEntity || "", invoiceEntity: item.invoiceEntity || "", companyEntityId: item.companyEntityId || "", invoiceEntityId: item.invoiceEntityId || "", invoiceEntityType: item.invoiceEntityType || "", invoiceDate: dateInputValue(item.invoiceDate), receivableDate: dateInputValue(item.receivableDate), actualReceiptDate: dateInputValue(item.actualReceiptDate), invoiceNo: item.invoiceNo || "", invoiceTotal: String(item.invoiceTotal), taxRate: String(item.taxRate), currency: item.currency, exchangeRate: String(item.exchangeRate), isPaid: item.isPaid, isInvoiced: item.isInvoiced }; }
...
function toInvoice(value: Record<string, string | boolean> | undefined) { return { type: value?.type || "cost", accountPeriod: dateInputValue(value?.accountPeriod), accountingDate: dateInputValue(value?.accountingDate), companyEntity: value?.companyEntity || "", invoiceEntity: value?.invoiceEntity || "", companyEntityId: value?.companyEntityId || "", invoiceEntityId: value?.invoiceEntityId || "", invoiceEntityType: value?.invoiceEntityType || "", invoiceDate: dateInputValue(value?.invoiceDate), receivableDate: dateInputValue(value?.receivableDate), actualReceiptDate: dateInputValue(value?.actualReceiptDate), invoiceNo: value?.invoiceNo || "", invoiceTotal: Number(value?.invoiceTotal || 0), taxRate: Number(value?.taxRate || 0), currency: value?.currency || "CNY", exchangeRate: Number(value?.exchangeRate || 1), isPaid: Boolean(value?.isPaid), isInvoiced: Boolean(value?.isInvoiced) }; }
...
function TableScroll({ children, tableKey }: { children: ReactNode; tableKey: string }) { return <StickyTable className="table-scroll table-viewport overflow-auto" tableKey={tableKey}>{children}</StickyTable>; }
function Empty({ colSpan, text }: { colSpan: number; text: string }) { return <tr><td className="px-4 py-10 text-center text-ink-3" colSpan={colSpan}>{text}</td></tr>; }
function Info({ label, value, link }: { label: string; value: string; link?: () => void }) { return <div><span className="block text-xs text-ink-3">{label}</span>{link ? <button className="mt-1 font-normal text-primary hover:underline" type="button" onClick={link}>{value}</button> : <strong className="mt-1 block font-normal text-ink">{value}</strong>}</div>; }
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) { return <button className={`border-b-2 px-4 py-3 text-sm ${active ? "border-primary text-primary" : "border-transparent text-ink-2"}`} type="button" onClick={onClick}>{children}</button>; }
function Field({ label, required, children }: { label: string; required?: boolean; children: ReactNode }) { return <div className="min-w-0"><span className="mb-1 block text-xs text-ink-2">{label}{required ? <span className="ml-0.5 text-danger">*</span> : null}</span>{children}</div>; }
function PriceType({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <Select value={value} onChange={(event) => onChange(event.target.value)}><option value="tax_excluded">未税价</option><option value="tax_included">含税价</option></Select>; }
...
function Toggle({ checked, onChange, label, disabled = false }: { checked: boolean; onChange: (checked: boolean) => void; label: string; disabled?: boolean }) {
  return <button className="inline-flex h-9 items-center gap-2 rounded border border-line bg-white px-2.5 text-sm text-ink-2 disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled} type="button" role="switch" aria-checked={checked} onClick={() => onChange(!checked)}><span className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${checked ? "bg-success-strong" : "bg-ink-4"}`}><span className={`h-4 w-4 rounded-full bg-white shadow transition-transform ${checked ? "translate-x-4" : "translate-x-0.5"}`} /></span><span>{checked ? `已${label}` : `未${label}`}</span></button>;
}
...
function PurchaseInput({ item, draft, field, onChange }: { item: Item; draft: Record<string, string>; field: string; onChange: (value: string) => void }) {
  if (["purchaseQty", "purchaseUnitPrice", "taxRate"].includes(field)) return <NumberInput className="min-w-[92px]" value={draft[field] || ""} onChange={(text) => onChange(text)} />;
  if (field === "currency") return <Select value={draft.currency || item.currency} onChange={(event) => onChange(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</Select>;
...
function EntryInput({ entry, value, field, kind, onChange }: { entry: Expense | Sale; value: Record<string, string>; field: string; kind: "expense" | "sale"; onChange: (value: string) => void }) {
  if (["amount", "taxRate"].includes(field)) return <NumberInput value={value[field] || ""} onChange={(text) => onChange(text)} />;
  if (field === "currency") return <Select value={value.currency} onChange={(event) => onChange(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</Select>;
  if (field === "priceType") return <PriceType value={value.priceType} onChange={onChange} />;
  if (field === "type" && kind === "expense") return <Select value={value.type} onChange={(event) => onChange(event.target.value)}><option value="first_mile_freight">头程运费</option><option value="customs_fee">清关费</option><option value="labor_fee">人力费</option><option value="equipment_service_fee">设备服务费</option><option value="other">其他</option></Select>;
...
function InvoiceInput({ invoice, value, field, partners, onChange }: { invoice: Invoice; value: Record<string, string | boolean>; field: string; partners: InvoicePartnerSet; onChange: (value: string | boolean, patch?: Record<string, string | boolean>) => void }) {
  if (field === "type") return <Select value={String(value.type)} onChange={(event) => onChange(event.target.value)}><option value="cost">成本</option><option value="income">收入</option></Select>;
  if (field === "currency") return <Select value={String(value.currency || invoice.currency)} onChange={(event) => onChange(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</Select>;
  if (field === "companyEntity") return <PartnerLookup options={partners.undertakingUnits} selectedId={value.companyEntityId} selectedName={value.companyEntity} onChange={(selected) => onChange(selected.name, { companyEntityId: selected.id, companyEntity: selected.name })} />;
  if (field === "invoiceEntity") return <PartnerLookup options={[...partners.suppliers.map((row) => ({ ...row, partnerType: "supplier" })), ...partners.customers.map((row) => ({ ...row, partnerType: "customer" }))]} selectedId={value.invoiceEntityId} selectedName={value.invoiceEntity} onChange={(selected) => onChange(selected.name, { invoiceEntityId: selected.id, invoiceEntity: selected.name, invoiceEntityType: selected.partnerType || "" })} />;
  ...
  if (["accountPeriod", "accountingDate", "invoiceDate", "receivableDate", "actualReceiptDate"].includes(field)) return <Input type="date" value={dateInputValue(value[field])} onChange={(event) => onChange(event.target.value)} />;
  return <Input value={String(value[field] ?? "")} onChange={(event) => onChange(event.target.value)} />;
}

type PartnerOption = Row & { partnerType?: string };
