...
function PurchaseTable({ title, tableKey, rows, selected, itemDrafts, editingRows, project, busy, importingItems, selectable, onSelect, onChange, onOrder, onImportItems, onEdit, onSave, onReturn, state, onState, onPage, onPageSize }: PurchaseTableProps) {
  ...
  const action = selectable ? <div className="flex flex-wrap justify-end gap-2"><a className="inline-flex h-9 items-center gap-1 rounded border border-line bg-white px-3 text-sm text-ink-2 hover:border-ink-4" href={"/api/po/settlement-projects/" + encodeURIComponent(project.id) + "/items/template"}><Download size={15} />下载模板</a><a className="inline-flex h-9 items-center gap-1 rounded border border-line bg-white px-3 text-sm text-ink-2 hover:border-ink-4" href={"/api/po/settlement-projects/" + encodeURIComponent(project.id) + "/items/export"}><Download size={15} />导出未采购</a><Button disabled={busy || importingItems} onClick={() => importInputRef.current?.click()}><FileUp size={15} />{importingItems ? "导入中..." : "导入未采购"}</Button><input ref={importInputRef} className="hidden" type="file" accept=".xlsx,.xls" onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ""; if (file) onImportItems?.(file); }} /><Button tone="primary" disabled={busy || !selected.length} onClick={onOrder}><Check size={15} />确认采购</Button></div> : undefined;
...
function InvoiceFormFields({ draft, partners, onChange }: { draft: Record<string, string | boolean>; partners: InvoicePartnerSet; onChange: (key: string, value: string | boolean) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-b border-line-soft bg-[#fcfcfd] p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8">
...
function statusClass(status: string) { return status === "closed" ? "bg-[#dcfce7] text-[#166534]" : status === "acceptance_completed" ? "bg-[#e0f2fe] text-[#0369a1]" : status === "accepting" ? "bg-[#e0e7ff] text-[#4338ca]" : status === "procurement_completed" ? "bg-[#dbeafe] text-[#1d4ed8]" : "bg-[#fef3c7] text-[#92400e]"; }
