...
function InvoiceFormFields({ draft, partners, onChange }: { draft: Record<string, string | boolean>; partners: InvoicePartnerSet; onChange: (key: string, value: string | boolean) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-b border-line-soft bg-surface-2 p-4 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-8">
      <Field label="类型"><select className="h-9 w-full rounded border border-line bg-white px-2" value={String(draft.type)} onChange={(event) => onChange("type", event.target.value)}><option value="cost">成本</option><option value="income">收入</option></select></Field>
      ...
      <Field label="发票币种"><select className="h-9 w-full rounded border border-line bg-white px-2" value={String(draft.currency)} onChange={(event) => onChange("currency", event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select></Field>
...
function PriceType({ value, onChange }: { value: string; onChange: (value: string) => void }) { return <select className="h-9 rounded border border-line bg-white px-2" value={value} onChange={(event) => onChange(event.target.value)}><option value="tax_excluded">未税价</option><option value="tax_included">含税价</option></select>; }
...
function PurchaseInput({ item, draft, field, onChange }: { item: Item; draft: Record<string, string>; field: string; onChange: (value: string) => void }) {
  ...
  if (field === "currency") return <select className="h-9 rounded border border-line bg-white px-2" value={draft.currency || item.currency} onChange={(event) => onChange(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select>;
...
function EntryInput({ entry, value, field, kind, onChange }: { entry: Expense | Sale; value: Record<string, string>; field: string; kind: "expense" | "sale"; onChange: (value: string) => void }) {
  ...
  if (field === "currency") return <select className="h-9 rounded border border-line" value={value.currency} onChange={(event) => onChange(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select>;
  if (field === "priceType") return <PriceType value={value.priceType} onChange={onChange} />;
  if (field === "type" && kind === "expense") return <select className="h-9 rounded border border-line" value={value.type} onChange={(event) => onChange(event.target.value)}><option value="first_mile_freight">头程运费</option><option value="customs_fee">清关费</option><option value="labor_fee">人力费</option><option value="equipment_service_fee">设备服务费</option><option value="other">其他</option></select>;
...
function InvoiceInput({ invoice, value, field, partners, onChange }: { invoice: Invoice; value: Record<string, string | boolean>; field: string; partners: InvoicePartnerSet; onChange: (value: string | boolean, patch?: Record<string, string | boolean>) => void }) {
  if (field === "type") return <select className="h-9 rounded border border-line" value={String(value.type)} onChange={(event) => onChange(event.target.value)}><option value="cost">成本</option><option value="income">收入</option></select>;
  if (field === "currency") return <select className="h-9 rounded border border-line" value={String(value.currency || invoice.currency)} onChange={(event) => onChange(event.target.value)}>{currencies.map((currency) => <option key={currency}>{currency}</option>)}</select>;
