...
const columns: Record<TableKey, Column[]> = {
  ...
  invoices: [
    ...
    { key: "currency", label: "发票币种", sortable: true, filterable: true }, { key: "receivableDate", label: "应收日期", type: "date", sortable: true, filterable: true }, { key: "isPaid", label: "是否支付", sortable: true, filterable: true }, { key: "isInvoiced", label: "是否开票", sortable: true, filterable: true }, { key: "createdAt", label: "创建时间", type: "date", sortable: true, filterable: true }, { key: "updatedAt", label: "更新时间", type: "date", sortable: true, filterable: true },
  ],
  attachments: [
...
function InvoiceSection({ invoices, allInvoices, pageSize, draft, editing, partners, busy, readOnly, attachmentDescriptions, onAttachmentDescriptionChange, onUpload, onDeleteAttachment, state, onState, onPage, onPageSize, onDraftChange, onEditingChange, onEdit, onAdd, onSave, onDelete }: { invoices: PagedRows; allInvoices: Invoice[]; pageSize: number; draft: Record<string, string | boolean>; editing: Record<string, Record<string, string | boolean>>; partners: InvoicePartnerSet; busy: boolean; readOnly: boolean; attachmentDescriptions: Record<string, string>; onAttachmentDescriptionChange: (invoiceId: string, value: string) => void; onUpload: (invoiceId: string, event: ChangeEvent<HTMLInputElement>) => void; onDeleteAttachment: (id: string) => void; state: TableState; onState: (patch: Partial<TableState>) => void; onPage: (page: number) => void; onPageSize: (pageSize: number) => void; onDraftChange: (key: string, value: string | boolean) => void; onEditingChange: (id: string, key: string, value: string | boolean, patch?: Record<string, string | boolean>) => void; onEdit: (invoice: Invoice) => void; onAdd: () => void; onSave: (id: string) => void; onDelete: (id: string) => void }) {
  const rows = invoices.rows as unknown as Invoice[];
  return <Panel><SectionTitle title="发票管理" action={<Button disabled={readOnly} tone="primary" onClick={onAdd}><FilePlus2 size={15} />新增发票</Button>} /><InvoiceFormFields draft={draft} partners={partners} onChange={onDraftChange} /><TableScroll tableKey="settlement-invoices"><table className="min-w-[2500px] border-collapse text-sm"><thead className="bg-[#f5f7fa]"><tr>{columns.invoices.map((column) => <th className="whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3 text-left font-medium" key={column.key}><ColumnHeader tableKey="invoices" column={column} rows={allInvoices as unknown as TableRow[]} state={state} onState={onState} /></th>)}<th>发票总额</th><th>发票不含税总额</th><th>税率(%)</th><th>发票税金</th><th>发票汇率</th><th>USD金额</th><th>发票附件</th><th>操作</th></tr></thead><tbody>{rows.map((invoice) => { const isEditing = Boolean(editing[invoice.id]); const value = editing[invoice.id] || draftInvoice(invoice); const preview = invoiceBreakdown(invoice, value); const attachments = invoice.attachments ?? []; const onInvoiceChange = (field: string, next: string | boolean, patch?: Record<string, string | boolean>) => onEditingChange(invoice.id, field, next, patch); return <tr className="hover:bg-[#fafafa]" key={invoice.id}>{columns.invoices.map((column) => <td className="max-w-[210px] truncate whitespace-nowrap border-b border-r border-[#ebeef5] px-3 py-3" key={column.key}>{isEditing && editableInvoiceField(column.key) ? <InvoiceInput invoice={invoice} value={value} field={column.key} partners={partners} onChange={(next, patch) => onInvoiceChange(column.key, next, patch)} /> : formatColumnValue(column.key, invoice[column.key as keyof Invoice], column.type)}</td>)}<td className="numeric-cell border-b border-r border-[#ebeef5] px-3 py-3">{isEditing ? <InvoiceInput invoice={invoice} value={value} field="invoiceTotal" partners={partners} onChange={(next, patch) => onInvoiceChange("invoiceTotal", next, patch)} /> : money(invoice.invoiceTotal)}</td><td className="numeric-cell border-b border-r border-[#ebeef5] px-3 py-3">{money(preview.invoiceTaxExcludedTotal)}</td><td className="numeric-cell border-b border-r border-[#ebeef5] px-3 py-3">{isEditing ? <InvoiceInput invoice={invoice} value={value} field="taxRate" partners={partners} onChange={(next, patch) => onInvoiceChange("taxRate", next, patch)} /> : money(invoice.taxRate)}</td><td className="numeric-cell border-b border-r border-[#ebeef5] px-3 py-3">{money(preview.invoiceTaxAmount)}</td><td className="numeric-cell border-b border-r border-[#ebeef5] px-3 py-3">{isEditing ? <InvoiceInput invoice={invoice} value={value} field="exchangeRate" partners={partners} onChange={(next, patch) => onInvoiceChange("exchangeRate", next, patch)} /> : money(invoice.exchangeRate)}</td><td className="numeric-cell border-b border-r border-[#ebeef5] px-3 py-3">{money(preview.usdAmount)}</td><td className="border-b border-r border-[#ebeef5] px-3 py-3"><div className="space-y-2">{attachments.map((attachment) => <div className="flex max-w-[280px] items-center gap-2" key={attachment.id}><span className="min-w-0 flex-1 truncate" title={attachment.fileName}>{attachment.fileName}</span><a className="inline-flex h-8 w-8 shrink-0 items-center justify-center text-[#1890ff] hover:text-[#147dcc]" aria-label="下载附件" title="下载附件" href={`/api/po/settlement-projects/${attachment.projectId}/invoices/${invoice.id}/attachments/${attachment.id}/download`}><Download size={15} /></a>{!readOnly ? <button className="inline-flex h-8 w-8 shrink-0 items-center justify-center text-[#f56c6c] hover:text-[#ff4949]" type="button" aria-label="删除附件" title="删除附件" onClick={() => onDeleteAttachment(attachment.id)}><Trash2 size={15} /></button> : null}</div>)}{!readOnly ? <div className="flex items-center gap-2"><Input className="min-w-[150px]" placeholder="附件说明" value={attachmentDescriptions[invoice.id] ?? ""} onChange={(event) => onAttachmentDescriptionChange(invoice.id, event.target.value)} /><label className="inline-flex h-9 shrink-0 cursor-pointer items-center gap-1 rounded border border-[#1890ff] px-3 text-sm text-[#1890ff]"><Paperclip size={15} />上传<input className="hidden" type="file" onChange={(event) => void onUpload(invoice.id, event)} /></label></div> : null}{!attachments.length && readOnly ? <span className="text-[#909399]">暂无附件</span> : null}</div></td><td className="whitespace-nowrap border-b border-[#ebeef5] px-3 py-3">{isEditing ? <><Button disabled={busy} tone="primary" onClick={() => onSave(invoice.id)}><Save size={14} />保存</Button></> : <Button disabled={readOnly || busy} onClick={() => onEdit(invoice)}><Pencil size={14} />修改</Button>}<Button className="ml-2" tone="danger" disabled={readOnly || busy} onClick={() => onDelete(invoice.id)}><Trash2 size={15} />删除</Button></td></tr>; })}{!rows.length ? <Empty colSpan={columns.invoices.length + 8} text="暂无发票" /> : null}</tbody></table></TableScroll><PaginationBar page={invoices.page} pageSize={invoices.pageSize} total={invoices.total} onPageChange={onPage} onPageSizeChange={onPageSize} /></Panel>;
}
...
function draftInvoice(item: Invoice) { return { type: item.type, accountPeriod: item.accountPeriod || "", accountingDate: item.accountingDate || "", companyEntity: item.companyEntity || "", invoiceEntity: item.invoiceEntity || "", companyEntityId: item.companyEntityId || "", invoiceEntityId: item.invoiceEntityId || "", invoiceEntityType: item.invoiceEntityType || "", invoiceDate: item.invoiceDate || "", receivableDate: item.receivableDate || "", invoiceNo: item.invoiceNo || "", invoiceTotal: String(item.invoiceTotal), taxRate: String(item.taxRate), currency: item.currency, exchangeRate: String(item.exchangeRate), isPaid: item.isPaid, isInvoiced: item.isInvoiced }; }
...
function toInvoice(value: Record<string, string | boolean> | undefined) { return { type: value?.type || "cost", accountPeriod: value?.accountPeriod || "", accountingDate: value?.accountingDate || "", companyEntity: value?.companyEntity || "", invoiceEntity: value?.invoiceEntity || "", companyEntityId: value?.companyEntityId || "", invoiceEntityId: value?.invoiceEntityId || "", invoiceEntityType: value?.invoiceEntityType || "", invoiceDate: value?.invoiceDate || "", receivableDate: value?.receivableDate || "", invoiceNo: value?.invoiceNo || "", invoiceTotal: Number(value?.invoiceTotal || 0), taxRate: Number(value?.taxRate || 0), currency: value?.currency || "CNY", exchangeRate: Number(value?.exchangeRate || 1), isPaid: Boolean(value?.isPaid), isInvoiced: Boolean(value?.isInvoiced) }; }
...
function InvoiceInput({ invoice, value, field, partners, onChange }: { invoice: Invoice; value: Record<string, string | boolean>; field: string; partners: InvoicePartnerSet; onChange: (value: string | boolean, patch?: Record<string, string | boolean>) => void }) {
  ...
  if (["accountPeriod", "accountingDate", "invoiceDate", "receivableDate"].includes(field)) return <Input type="date" value={String(value[field] || "").slice(0, 10)} onChange={(event) => onChange(event.target.value)} />;
...
function PartnerLookup({ options, selectedId, selectedName, onChange }: { options: PartnerOption[]; selectedId: unknown; selectedName: unknown; onChange: (selected: { id: string; name: string; partnerType?: string }) => void }) {
  ...
  const selectedValue = selectedOption ? partnerOptionId(selectedOption) : selected;
  const selectedLabel = selectedOption ? partnerOptionName(selectedOption) : savedName || "请选择";
  return <div className="space-y-1"><Input className="w-full" placeholder="输入编码、简称或名称搜索" value={keyword} onChange={(event) => setKeyword(event.target.value)} /><select className="h-9 w-full rounded border border-[#dcdfe6] bg-white px-2" value={selectedValue} onChange={(event) => { const row = renderedOptions.find((candidate) => partnerOptionId(candidate) === event.target.value); onChange({ id: event.target.value, name: row ? partnerOptionName(row) : "", partnerType: row?.partnerType }); }}><option value="">{selectedLabel}</option>{renderedOptions.map((row) => <option key={`${row.partnerType || "unit"}-${partnerOptionId(row)}`} value={partnerOptionId(row)}>{partnerOptionCode(row) ? `${partnerOptionCode(row)} - ` : ""}{partnerOptionName(row)}</option>)}</select></div>;
}
