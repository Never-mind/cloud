...
export function SettlementProjectPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-medium text-ink">项目结算</h1>
        <p className="mt-1 text-sm text-ink-3">已确认报价单自动生成项目结算主单，采购、收入和发票明细独立维护。</p>
      </div>
      <Panel>
        <div className="flex flex-wrap items-center gap-2 border-b border-line-soft p-3">
          ...
          <a className="inline-flex h-8 shrink-0 items-center gap-1 whitespace-nowrap rounded border border-warning-deep bg-white px-3 text-sm text-[#8a5200] transition-colors hover:border-[#d48806] hover:bg-warning-soft" href="/api/po/settlement-projects/export"><FileDown size={14} />导出 Excel</a>
        </div>
         ...
         <table className="min-w-[2400px] border-collapse text-sm">
            <thead className="bg-canvas"><tr>{columns.map(([field, label]) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={field}><TableColumnMenu column={{ key: field, label, sortable: true, filterable: true }} sortOrder={sortField === field ? sortOrder : ""} filterValues={columnFilters[field] ?? []} loadOptions={(optionKeyword) => loadOptions(field, optionKeyword)} onSort={(order) => { setPage(1); setSortField(order ? field : ""); setSortOrder(order); }} onFilter={(values) => { setPage(1); setColumnFilters((current) => ({ ...current, [field]: values })); }} /></th>)}<th className="border-b border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead>
             <tbody>{loading ? <tr><td className="px-4 py-12 text-center text-ink-3" colSpan={columns.length + 1}><TableSkeleton /></td></tr> : rows.map((row) => <tr key={row.id}>
              ...
              <td className="whitespace-nowrap px-3 py-3"><button className="mr-2 inline-flex h-8 w-8 items-center justify-center text-ink-2 hover:text-primary" type="button" aria-label="查看" title="查看" onClick={() => openRoute(`/po/settlement-projects/${encodeURIComponent(row.id)}?returnTo=%2Fpo%2Fsettlement-projects`, "项目结算详情")}><Eye size={16} /></button>{row.status !== "closed" ? <button className="inline-flex h-8 w-8 items-center justify-center text-danger hover:text-danger" type="button" aria-label="删除" title="删除" onClick={() => void deleteProject(row.id)}><Trash2 size={16} /></button> : null}</td>
              </tr>)}{!loading && !rows.length ? <tr><td className="px-4 py-12 text-center text-ink-3" colSpan={columns.length + 1}>暂无项目结算。报价单确认后会自动生成。</td></tr> : null}</tbody>
          </table>
        </StickyTable>
