...
function RelatedSection({ config, ownerId, editing, enabled, persisted, pendingRows, onPendingRowsChange, onError }: {
  ...
  return (
    <section className="border-t border-line-soft p-5">
      ...
      {editingId !== null ? <form className="mt-4 border border-info-border bg-info-soft p-4" onSubmit={save}><div className="mb-3 flex items-center"><strong className="text-sm text-ink">{editingId ? `编辑${isBank ? "银行账户" : "联系人"}` : `新增${isBank ? "银行账户" : "联系人"}`}</strong><button className="ml-auto text-sm text-ink-3" type="button" onClick={() => setEditingId(null)}>取消</button></div><div className="grid gap-x-5 gap-y-4 md:grid-cols-2 lg:grid-cols-4">{fields.map((field) => <FieldEditor field={field} editing value={draft[field.key]} key={field.key} onChange={(value) => setDraft((current) => ({ ...current, [field.key]: value }))} />)}</div><div className="mt-4 flex justify-end"><Button tone="primary" type="submit" disabled={saving}><Save size={15} />{saving ? "保存中..." : "保存"}</Button></div></form> : null}
    </section>
  );
}
