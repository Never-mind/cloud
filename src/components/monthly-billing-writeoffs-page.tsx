...
import { monthlyBillingWriteOffDisplayColumns } from "@/lib/writeoff-export-columns";
...
const displayColumns = monthlyBillingWriteOffDisplayColumns.map((column) => ({ ...column, sortable: true, filterable: true }));
...
export function MonthlyBillingWriteOffsPage() {
  ...
  async function loadData(nextPage = page, nextPageSize = pageSizeRef.current, filters = appliedFilters, reuseKnownTotals = false) {
    ...
    try {
      ...
      setTotal(0);
      setTotalAmount(0);
      notify(error instanceof Error ? error.message : "月账单明细加载失败", "info");
    } finally {
  ...
  async function downloadExport() {
    setExporting(true);
    try {
      const response = await fetch(`/api/billing/monthly-writeoffs/export?${buildRequestParams(1, pageSizeRef.current, false, appliedFilters).toString()}`);
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data.error ?? "月账单明细导出失败");
      }
