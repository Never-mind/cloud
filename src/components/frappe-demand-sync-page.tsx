      ...
      <StickyTable className="max-h-[calc(100vh-280px)] overflow-auto" tableKey="frappe-demand-sync-ledger">
        <table className="min-w-[1100px] border-collapse text-sm">
          <thead className="bg-[#f5f7fa]"><tr>
            <th className="w-4 border-b border-r border-[#ebeef5] px-0 py-3 text-center font-medium [&>input]:h-3.5 [&>input]:w-3.5 [&>input]:align-middle"><input aria-label="全选需求单" checked={ledger.length > 0 && selectedOrders.length === ledger.length} type="checkbox" onChange={(event) => setSelectedOrders(event.target.checked ? ledger.map((row) => row.sourceOrderId) : [])} /></th>
          ...
          <tbody>
            {ledger.map((row) => <Fragment key={row.sourceOrderId}>
              <tr className="hover:bg-[#fafafa]">
                <td className="w-4 border-b border-r border-[#ebeef5] px-0 py-3 text-center [&>input]:h-3.5 [&>input]:w-3.5 [&>input]:align-middle"><input aria-label={`选择 ${row.sourceOrderId}`} checked={selectedOrders.includes(row.sourceOrderId)} type="checkbox" onChange={() => setSelectedOrders((current) => current.includes(row.sourceOrderId) ? current.filter((id) => id !== row.sourceOrderId) : [...current, row.sourceOrderId])} /></td>
