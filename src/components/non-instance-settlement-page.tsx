...
export function NonInstanceSettlementPage({ countries }: { countries: Country[] }) {
  ...
  return <Panel>
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft p-4"><div><h1 className="font-medium text-ink">非实例费用结差</h1><p className="mt-1 text-sm text-ink-3">先形成已确认的来源单，再由“结差结算单”统一汇集实例与非实例费用。</p></div><div className="flex flex-wrap gap-2"><Button onClick={downloadTemplate}><Download size={15} />下载模板</Button><Button onClick={() => { setImportOpen(true); setImportFailures([]); setImportTotal(0); setImportedCount(0); }}><FileUp size={15} />导入明细</Button><Button onClick={exportCurrentLines}><Download size={15} />导出明细</Button><Button tone="primary" disabled={saving} onClick={() => void saveDraft()}><ListChecks size={15} />保存草稿</Button></div></div>
    <div className="grid gap-3 border-b border-line-soft p-4 md:grid-cols-2 xl:grid-cols-4"><Input placeholder="结差单名称" value={title} onChange={(event) => setTitle(event.target.value)} /><select className="h-9 rounded border border-line bg-white px-2 text-sm" value={countryCode} onChange={(event) => setCountryCode(event.target.value)}><option value="">选择国家</option>{countries.map((country) => <option key={country.code} value={country.code}>{country.code} {country.nameZh ? `- ${country.nameZh}` : ""}</option>)}</select><select className="h-9 rounded border border-line bg-white px-2 text-sm" value={currency} onChange={(event) => setCurrency(event.target.value)}>{NON_INSTANCE_SETTLEMENT_CURRENCIES.map((item) => <option key={item} value={item}>{item}</option>)}</select><Input placeholder="来源文件名（可选）" value={sourceFileName} onChange={(event) => setSourceFileName(event.target.value)} /><label className="grid gap-1 text-xs text-ink-2"><span>结差期间开始</span><Input type="date" value={periodStart} onChange={(event) => setPeriodStart(event.target.value)} /></label><label className="grid gap-1 text-xs text-ink-2"><span>结差期间结束</span><Input type="date" value={periodEnd} onChange={(event) => setPeriodEnd(event.target.value)} /></label><label className="grid gap-1 text-xs text-ink-2 xl:col-span-2"><span>备注</span><Textarea className="min-h-9" value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div>
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft p-4"><div><h2 className="font-medium text-ink">明细类型</h2><p className="mt-1 text-sm text-ink-3">同一张来源单使用同一类字段，后续可与其他类型来源单一起汇总。</p><p className="mt-1 max-w-4xl text-xs text-ink-2">{calculationFormula}</p></div><select className="h-9 min-w-[210px] rounded border border-line bg-white px-2 text-sm" value={expenseType} onChange={(event) => changeType(event.target.value)}>{NON_INSTANCE_EXPENSE_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></div>
    <StickyTable className="table-scroll overflow-auto" tableKey="non-instance-settlement-lines"><table className="w-full min-w-[1900px] border-collapse text-sm"><thead className="bg-canvas"><tr>{fields.map((field) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={field.key}>{field.label}{field.required ? <span className="ml-1 text-danger">*</span> : null}</th>)}<th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{lines.map((line, index) => <tr key={index}>{fields.map((field) => <td className="border-b border-r border-line-soft px-2 py-2" key={field.key}>{field.derived ? <div className="flex h-8 min-w-[130px] items-center rounded border border-[#e4e7ed] bg-canvas px-2 text-ink-2">{formatCalculatedValue(line[field.key])}</div> : field.key === "feeCurrency" || field.key === "priceConfirmation" || field.key === "confirmationResult" || field.key === "differenceNature" ? <select className="h-8 min-w-[105px] rounded border border-line bg-white px-2" value={line[field.key]} onChange={(event) => updateLine(index, field.key, event.target.value)}>{(field.key === "feeCurrency" ? NON_INSTANCE_SETTLEMENT_CURRENCIES : field.key === "differenceNature" ? ["CAPEX", "OPEX"] : ["YES", "NO"]).map((value) => <option value={value} key={value}>{value}</option>)}</select> : <input className="h-8 min-w-[110px] rounded border border-line px-2" inputMode={field.kind === "number" || field.kind === "percentage" ? "decimal" : undefined} min={field.kind === "number" || field.kind === "percentage" ? "0" : undefined} placeholder={field.kind === "percentage" ? "如 16 表示16%" : undefined} step={field.kind === "percentage" ? "0.01" : field.kind === "number" ? "0.0001" : undefined} type={field.kind === "date" ? "date" : field.kind === "number" || field.kind === "percentage" ? "number" : "text"} value={line[field.key]} onChange={(event) => updateLine(index, field.key, event.target.value)} />}</td>)}<td className="border-b border-r border-line-soft px-2 py-2"><button className="text-danger" aria-label="删除明细" onClick={() => setLines((current) => current.filter((_, lineIndex) => lineIndex !== index))}><Trash2 size={17} /></button></td></tr>)}</tbody></table></StickyTable>
    <div className="flex flex-wrap items-center gap-2 p-4"><Button onClick={() => setLines((current) => [...current, createBlankNonInstanceSettlementLine(expenseType)])}><Plus size={15} />新增明细</Button><span className="text-xs text-ink-3">带 * 的字段为必填；灰底字段由系统实时计算，导入时也会重新计算。</span></div>
    {importOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 p-4"><div className="max-h-[85vh] w-full max-w-3xl overflow-auto rounded border border-line-soft bg-white shadow-xl"><div className="flex items-center justify-between border-b border-line-soft px-5 py-4"><div><h2 className="font-medium text-ink">导入{expenseType}明细</h2><p className="mt-1 text-sm text-ink-3">先下载当前类型模板；上传后只填入页面，点击“保存草稿”才写入数据库。</p></div><button aria-label="关闭导入窗口" className="text-ink-3 hover:text-ink" onClick={() => setImportOpen(false)}><X size={20} /></button></div><div className="space-y-4 p-5"><div className="flex flex-wrap gap-2"><Button onClick={downloadTemplate}><Download size={15} />下载当前模板</Button><Button tone="primary" onClick={() => fileInputRef.current?.click()}><FileUp size={15} />选择 Excel 文件</Button><input accept=".xlsx,.xls,.csv" className="hidden" ref={fileInputRef} type="file" onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ""; if (file) void importFile(file); }} /></div>{importFileName && <div className="border border-line-soft bg-surface-2 px-3 py-2 text-sm text-ink-2">文件：{importFileName}。共读取 {importTotal} 行，成功填入 {importedCount} 行，失败 {importFailures.length} 行。</div>}{importFailures.length > 0 && <div className="overflow-auto border border-danger-border"><div className="flex items-center justify-between bg-danger-soft px-3 py-2 text-sm text-danger"><span>以下行未填入页面，请修正后重新导入。</span><Button className="h-7" onClick={downloadImportErrors}><Download size={14} />下载错误报告</Button></div><table className="w-full border-collapse text-sm"><thead className="bg-canvas"><tr><th className="border-b border-r border-line-soft px-3 py-2 text-left">Excel行号</th><th className="border-b border-line-soft px-3 py-2 text-left">错误原因</th></tr></thead><tbody>{importFailures.map((failure) => <tr key={`${failure.rowNumber}-${failure.error}`}><td className="border-b border-r border-line-soft px-3 py-2">{failure.rowNumber}</td><td className="border-b border-line-soft px-3 py-2">{failure.error}</td></tr>)}</tbody></table></div>}<div className="flex justify-end"><Button onClick={() => setImportOpen(false)}>{importedCount ? "完成并查看明细" : "关闭"}</Button></div></div></div></div>}
  </Panel>;
}
