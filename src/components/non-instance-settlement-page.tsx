...
export function NonInstanceSettlementPage({ countries }: { countries: Country[] }) {
  ...
  return <Panel>
    ...
    <div className="grid gap-3 border-b border-line-soft p-4 md:grid-cols-2 xl:grid-cols-4"><Input placeholder="结差单名称" value={title} onChange={(event) => setTitle(event.target.value)} /><Select value={countryCode} onChange={(event) => setCountryCode(event.target.value)}><option value="">选择国家</option>{countries.map((country) => <option key={country.code} value={country.code}>{country.code} {country.nameZh ? `- ${country.nameZh}` : ""}</option>)}</Select><Select value={currency} onChange={(event) => setCurrency(event.target.value)}>{NON_INSTANCE_SETTLEMENT_CURRENCIES.map((item) => <option key={item} value={item}>{item}</option>)}</Select><Input placeholder="来源文件名（可选）" value={sourceFileName} onChange={(event) => setSourceFileName(event.target.value)} /><label className="grid gap-1 text-xs text-ink-2"><span>结差期间开始</span><Input type="date" value={periodStart} onChange={(event) => setPeriodStart(event.target.value)} /></label><label className="grid gap-1 text-xs text-ink-2"><span>结差期间结束</span><Input type="date" value={periodEnd} onChange={(event) => setPeriodEnd(event.target.value)} /></label><label className="grid gap-1 text-xs text-ink-2 xl:col-span-2"><span>备注</span><Textarea className="min-h-9" value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div>
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line-soft p-4"><div><h2 className="font-medium text-ink">明细类型</h2><p className="mt-1 text-sm text-ink-3">同一张来源单使用同一类字段，后续可与其他类型来源单一起汇总。</p><p className="mt-1 max-w-4xl text-xs text-ink-2">{calculationFormula}</p></div><Select className="min-w-[210px]" value={expenseType} onChange={(event) => changeType(event.target.value)}>{NON_INSTANCE_EXPENSE_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</Select></div>
    <StickyTable className="table-scroll table-viewport overflow-auto" tableKey="non-instance-settlement-lines"><table className="w-full min-w-[1900px] border-collapse text-sm"><thead className="bg-canvas"><tr>{fields.map((field) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={field.key}>{field.label}{field.required ? <span className="ml-1 text-danger">*</span> : null}</th>)}<th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium">操作</th></tr></thead><tbody>{lines.map((line, index) => <tr key={index}>{fields.map((field) => <td className="border-b border-r border-line-soft px-2 py-2" key={field.key}>{field.derived ? <div className="flex h-8 min-w-[130px] items-center rounded border border-line bg-canvas px-2 text-ink-2">{formatCalculatedValue(line[field.key])}</div> : field.key === "feeCurrency" || field.key === "priceConfirmation" || field.key === "confirmationResult" || field.key === "differenceNature" ? <Select className="min-w-[105px]" value={line[field.key]} onChange={(event) => updateLine(index, field.key, event.target.value)}>{(field.key === "feeCurrency" ? NON_INSTANCE_SETTLEMENT_CURRENCIES : field.key === "differenceNature" ? ["CAPEX", "OPEX"] : ["YES", "NO"]).map((value) => <option value={value} key={value}>{value}</option>)}</Select> : <input className="h-8 min-w-[110px] rounded border border-line px-2" inputMode={field.kind === "number" || field.kind === "percentage" ? "decimal" : undefined} min={field.kind === "number" || field.kind === "percentage" ? "0" : undefined} placeholder={field.kind === "percentage" ? "如 16 表示16%" : undefined} step={field.kind === "percentage" ? "0.01" : field.kind === "number" ? "0.0001" : undefined} type={field.kind === "date" ? "date" : field.kind === "number" || field.kind === "percentage" ? "number" : "text"} value={line[field.key]} onChange={(event) => updateLine(index, field.key, event.target.value)} />}</td>)}<td className="border-b border-r border-line-soft px-2 py-2"><button className="text-danger" aria-label="删除明细" onClick={() => setLines((current) => current.filter((_, lineIndex) => lineIndex !== index))}><Trash2 size={17} /></button></td></tr>)}</tbody></table></StickyTable>
