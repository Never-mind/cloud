...
function EditablePriceField({ label, value, manual, onChange, emphasized }: { label: string; value: number; manual: boolean; onChange: (value: number) => void; emphasized?: boolean }) {
  return <label className={`relative block border p-3 ${emphasized ? "border-info-border bg-[#f4f9ff]" : "border-line-soft bg-surface-2"}`}>
    ...
    <span className="mt-1 flex h-9 items-center border border-line bg-white focus-within:border-primary"><span className="pl-3 text-sm text-ink-3">USD</span><input aria-label={label} className="h-full min-w-0 flex-1 bg-transparent px-2 text-sm font-medium text-ink outline-none" min="0" step="0.01" type="number" value={String(value)} onChange={(event) => onChange(Number(event.target.value || 0))} /></span>
  </label>;
}
...
function NumberField({ label, value, decimals, onChange }: { label: string; value: number; decimals: number; onChange: (value: number) => void }) { return <label className="min-w-0"><span className="mb-1 block text-xs text-ink-2">{label}</span><Input className="w-full" step={10 ** -decimals} type="number" value={String(value)} onChange={(event) => onChange(Number(event.target.value || 0))} /></label>; }
function PercentField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="min-w-0"><span className="mb-1 block text-xs text-ink-2">{label}</span><div className="relative"><Input className="w-full pr-9" step="0.01" type="number" value={String(value * 100)} onChange={(event) => onChange(Number(event.target.value || 0) / 100)} /><span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-xs text-ink-3">%</span></div></label>; }
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: Array<[string, string]>; onChange: (value: string) => void }) { return <label className="min-w-0"><span className="mb-1 block text-xs text-ink-2">{label}</span><select className="h-9 w-full rounded border border-line bg-white px-3 text-sm outline-none focus:border-primary" value={value} onChange={(event) => onChange(event.target.value)}>{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></label>; }
