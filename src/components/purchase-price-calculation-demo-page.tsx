...
import { Calculator, ChevronDown, ChevronUp, Clock3, Info, RotateCcw } from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start gap-3">
        <div>
          <div className="flex items-center gap-2"><h1 className="text-xl font-medium text-ink">算力采购服务费测算</h1><span className="border border-info-border bg-info-soft px-2 py-0.5 text-xs text-info">演示</span></div>
          <p className="mt-1 text-sm text-ink-3">采购单可使用 CNY 或 USD；测算统一换算为人民币 CAPEX 后，按算力服务费定价模板计算。</p>
        </div>
        <Button className="ml-auto" tone="primary" onClick={() => setDrawerOpen(true)}><Calculator size={15} />价格测算</Button>
      </div>

      <Panel>
        <div className="flex items-center justify-between border-b border-line-soft px-4 py-3"><div><div className="font-medium text-ink">采购订单明细</div><div className="mt-1 text-xs text-ink-3">PO-20260903-0001 · {defaults.countryName} · 采购币种 {purchase.purchaseCurrency}</div></div><span className="border border-info-border bg-info-soft px-2 py-1 text-xs text-info">草稿</span></div>
        <div className="table-scroll overflow-auto"><table className="w-full min-w-[1530px] border-collapse text-sm"><thead className="bg-canvas text-ink"><tr>{["行号", "设备编码", "B6 类型", "产品名称", "数量", "币种", "不含税单价", "税费加成", "CAPEX（不含 VAT，CNY）", "算力服务价格（1-24个月，含 VAT）", "算力服务价格（25-60个月，含 VAT）", "操作"].map((label) => <th className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3 text-left font-medium" key={label}>{label}</th>)}</tr></thead><tbody><tr className="hover:bg-surface-2"><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">1</td><td className="border-b border-r border-line-soft px-3 py-3 font-medium text-ink">{purchase.deviceCode}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{purchase.model}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{purchase.productName}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{purchase.quantity}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{purchase.purchaseCurrency}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{purchase.purchaseCurrency} {money(purchase.taxExcludedUnitPrice, 4)}</td><td className="whitespace-nowrap border-b border-r border-line-soft px-3 py-3">{purchase.purchaseCurrency} {money(purchase.taxSurcharge, 4)}</td><td className="border-b border-r border-line-soft px-3 py-3 font-medium text-ink">CNY {money(inputs.capexWithoutVatCny, 4)}</td><td className="border-b border-r border-line-soft px-3 py-3 font-medium text-primary">USD {money(result.first24VatIncluded)}</td><td className="border-b border-r border-line-soft px-3 py-3 font-medium text-success-dark">USD {money(result.next36VatIncluded)}</td><td className="border-b border-line-soft px-3 py-3"><button className="inline-flex h-8 w-8 items-center justify-center border border-info-border text-primary hover:bg-info-soft" title="算力服务费测算" type="button" onClick={() => setDrawerOpen(true)}><Calculator size={15} /></button></td></tr></tbody></table></div>
      </Panel>

      {drawerOpen ? <Drawer description={<>第 1 行 · {purchase.deviceCode} · {defaults.countryName} · 按 2026 年 6 月定价模板</>} onClose={() => setDrawerOpen(false)} title={<><Calculator className="text-primary" size={19} />算力服务费测算</>} widthClass="max-w-[700px]">
        <section><div className="mb-1 flex items-center justify-between"><SectionTitle title="本次算力服务价格" /><span className="text-xs text-ink-3">默认按采购币种换算，可手工调整</span></div><div className="grid grid-cols-2 gap-3"><EditablePriceField emphasized label="1-24 个月（转合同币种，含 VAT）" value={result.first24VatIncluded} manual={manualPrices.first24VatIncluded !== undefined} onChange={(value) => updateManualPrice("first24VatIncluded", value)} /><EditablePriceField emphasized label="25-60 个月（转合同币种，含 VAT）" value={result.next36VatIncluded} manual={manualPrices.next36VatIncluded !== undefined} onChange={(value) => updateManualPrice("next36VatIncluded", value)} /><ResultField label="1-24 个月（转合同币种，不含 VAT）" value={`USD ${money(result.first24NoVatContract)}`} /><ResultField label="25-60 个月（转合同币种，不含 VAT）" value={`USD ${money(result.next36NoVatContract)}`} /></div></section>
        <section><SectionTitle icon={<Clock3 size={15} />} title="最近实例合同价格" /><div className="overflow-hidden border border-line-soft"><table className="w-full border-collapse text-xs"><thead className="bg-canvas text-ink-2"><tr><th className="whitespace-nowrap px-3 py-3 text-left font-medium">合同</th><th className="whitespace-nowrap px-3 py-3 text-left font-medium">签署日期</th><th className="whitespace-nowrap px-3 py-3 text-right font-medium">前 24 月</th><th className="whitespace-nowrap px-3 py-3 text-right font-medium">后 36 月</th><th className="whitespace-nowrap px-3 py-3 text-right font-medium">本次前 24 月差额</th><th className="whitespace-nowrap px-3 py-3 text-right font-medium">本次后 36 月差额</th></tr></thead><tbody>{historicalContracts.map((contract) => <tr key={contract.version}><td className="border-t border-line-soft px-3 py-2 text-ink">{contract.version}</td><td className="border-t border-line-soft px-3 py-2">{contract.date}</td><td className="border-t border-line-soft px-3 py-2 text-right">USD {money(contract.first)}</td><td className="border-t border-line-soft px-3 py-2 text-right">USD {money(contract.next)}</td><td className="border-t border-line-soft px-3 py-2 text-right">{formatDifference(result.first24VatIncluded - contract.first)}</td><td className="border-t border-line-soft px-3 py-2 text-right">{formatDifference(result.next36VatIncluded - contract.next)}</td></tr>)}</tbody></table></div><div className="mt-2 text-xs text-ink-3">历史合同仅用于价格对比，不会自动覆盖本次服务费测算结果。</div></section>
        ...
        <section className="border border-line-soft"><button className="flex w-full items-center justify-between px-3 py-3 text-left text-sm font-medium text-ink hover:bg-surface-2" type="button" onClick={() => setShowFormula((value) => !value)}><span className="flex items-center gap-2"><Info className="text-ink-3" size={15} />表格计算公式</span>{showFormula ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</button>{showFormula ? <div className="space-y-2 border-t border-line-soft bg-surface-2 px-3 py-3 text-xs leading-5 text-ink-2"><div>1. 采购币种为 CNY 时，CAPEX 默认等于不含税单价 + 税费加成；采购币种为 USD 时，CAPEX 默认等于（不含税单价 + 税费加成）÷ 整机价转合同汇率。</div><div>2. CAPEX 合计 = CAPEX（不含 VAT）×（1 + Onsite/RMA 费率 + 年利率×资金占用月数÷12）。</div><div>3. DDP 价格 = CAPEX 合计 ×（1 + 运保清关费率）×（1 + 总代过手费率 + 其他税费率）。</div><div>4. OPEX = DDP 价格 - CAPEX 合计；OPEX 全额平均分摊至前 24 个月。</div><div>5. 前 24 月不含 VAT 服务费 = CAPEX 合计÷模板整机基准价×模板前 24 月基准服务费 + OPEX÷24。</div><div>6. 后 36 月不含 VAT 服务费 = CAPEX 合计÷模板整机基准价×模板后 36 月基准服务费。</div></div> : null}</section>
      </Drawer> : null}
    </div>
  );
}
...
function CurrencyConversionHint({ purchase, exchangeRate, defaultCapex, currentCapex, manualCapex }: { purchase: PurchaseLine; exchangeRate: number; defaultCapex: number; currentCapex: number; manualCapex: boolean }) {
  ...
  return <div className="mt-3 border border-info-border bg-info-soft px-3 py-2 text-xs leading-5 text-ink-2">{content}</div>;
}
...
function SectionTitle({ title, icon }: { title: string; icon?: React.ReactNode }) { return <div className="mb-3 flex items-center gap-2 text-sm font-medium text-ink">{icon}{title}</div>; }
function ReadOnlyField({ label, value }: { label: string; value: string }) { return <div><div className="mb-1 block text-xs text-ink-3">{label}</div><div className="flex h-9 items-center rounded border border-line-soft bg-canvas px-3 text-sm text-ink-2">{value}</div></div>; }
function EditablePriceField({ label, value, manual, onChange, emphasized }: { label: string; value: number; manual: boolean; onChange: (value: number) => void; emphasized?: boolean }) {
  return <label className={`relative block border p-3 ${emphasized ? "border-info-border bg-info-soft" : "border-line-soft bg-surface-2"}`}>
    <span className="flex items-center justify-between gap-2 text-xs text-ink-2"><span>{label}</span>{manual ? <span className="whitespace-nowrap text-warning-deep">手工</span> : <span className="whitespace-nowrap text-success-strong">自动</span>}</span>
    <span className="mt-1 flex h-9 items-center border border-line bg-white focus-within:border-primary"><span className="pl-3 text-sm text-ink-3">USD</span><NumberInput bare aria-label={label} className="h-full min-w-0 flex-1 bg-transparent px-2 text-sm font-medium text-ink outline-none" min="0" step="0.01" value={value} onChange={(text) => onChange(Number(text || 0))} /></span>
  </label>;
}
function ResultField({ label, value, emphasized }: { label: string; value: string; emphasized?: boolean }) { return <div className={`border p-3 ${emphasized ? "border-info-border bg-info-soft" : "border-line-soft bg-surface-2"}`}><div className="text-xs text-ink-3">{label}</div><div className={`mt-1 text-sm ${emphasized ? "font-medium text-primary" : "text-ink"}`}>{value}</div></div>; }
function NumberField({ label, value, decimals, onChange }: { label: string; value: number; decimals: number; onChange: (value: number) => void }) { return <label className="min-w-0"><span className="mb-1 block text-xs text-ink-2">{label}</span><NumberInput className="w-full" step={10 ** -decimals} value={value} onChange={(text) => onChange(Number(text || 0))} /></label>; }
function PercentField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="min-w-0"><span className="mb-1 block text-xs text-ink-2">{label}</span><div className="relative"><NumberInput className="w-full pr-9" step="0.01" value={value * 100} onChange={(text) => onChange(Number(text || 0) / 100)} /><span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-xs text-ink-3">%</span></div></label>; }
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: Array<[string, string]>; onChange: (value: string) => void }) { return <label className="min-w-0"><span className="mb-1 block text-xs text-ink-2">{label}</span><Select className="w-full" value={value} onChange={(event) => onChange(event.target.value)}>{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</Select></label>; }
