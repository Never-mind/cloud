...
export function PowerPriceCalculationDrawer({
  ...
  return (
    ...
    >
          ...
          {storageReady === false ? <div className="border border-warning-border-soft bg-warning-soft px-3 py-2 text-xs text-warning-deep">数据库尚未创建算力服务费测算字段。可以继续预览，但应用结果前请先执行本次迁移 SQL。</div> : null}
    </Drawer>
  );
}
...
function EditablePrice({ label, value, manual, onChange }: { label: string; value: number; manual: boolean; onChange: (value: number) => void }) { return <label className="border border-info-border bg-info-soft p-3"><span className="flex items-center justify-between gap-2 text-xs text-ink-2"><span>{label}</span><span className={manual ? "text-warning-deep" : "text-success-strong"}>{manual ? "手工" : "自动"}</span></span><span className="mt-1 flex h-9 items-center border border-line bg-white focus-within:border-primary"><span className="pl-3 text-sm text-ink-3">USD</span><NumberField bare aria-label={label} className="h-full min-w-0 flex-1 bg-transparent px-2 text-sm font-medium text-ink outline-none" min="0" step="0.01" value={value} onChange={(text) => onChange(Number(text || 0))} /></span></label>; }
