...
function EditablePrice({ label, value, manual, onChange }: { label: string; value: number; manual: boolean; onChange: (value: number) => void }) { return <label className="border border-info-border bg-[#f4f9ff] p-3"><span className="flex items-center justify-between gap-2 text-xs text-ink-2"><span>{label}</span><span className={manual ? "text-warning-deep" : "text-success-strong"}>{manual ? "手工" : "自动"}</span></span><span className="mt-1 flex h-9 items-center border border-line bg-white focus-within:border-primary"><span className="pl-3 text-sm text-ink-3">USD</span><input aria-label={label} className="h-full min-w-0 flex-1 bg-transparent px-2 text-sm font-medium text-ink outline-none" min="0" step="0.01" type="number" value={String(value)} onChange={(event) => onChange(Number(event.target.value || 0))} /></span></label>; }
