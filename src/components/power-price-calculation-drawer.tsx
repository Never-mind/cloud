    ...
    <>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[760px] flex-col border-l border-[#dcdfe6] bg-white shadow-2xl">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
          ...
          <section className="border border-[#ebeef5]"><button className="flex w-full items-center justify-between px-3 py-3 text-left text-sm font-medium text-[#303133] hover:bg-[#fafafa]" type="button" onClick={() => setShowFormula((value) => !value)}><span className="flex items-center gap-2"><Info className="text-[#909399]" size={15} />计算公式</span>{showFormula ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</button>{showFormula ? <div className="space-y-2 border-t border-[#ebeef5] bg-[#fafafa] px-3 py-3 text-xs leading-5 text-[#606266]"><div>1. CNY采购：CAPEX不含VAT = 不含税单价 + 税费加成金额。</div><div>2. USD采购：CAPEX不含VAT =（不含税单价 + 税费加成金额）÷ 整机价转合同汇率。</div><div>3. 资金占用费 = CAPEX不含VAT × 年资金费率 × 资金占用月数 ÷ 12。</div><div>4. CAPEX合计 = CAPEX不含VAT ×（1 + Onsite/RMA费率）+ 资金占用费。</div><div>5. DDP价格 = CAPEX合计 ×（1 + 运保清关费率）×（1 + 总代过手费率 + 其他税费率）。</div><div>6. OPEX = DDP价格 - CAPEX合计；OPEX全部平均分摊到前24个月。</div><div>7. 前24/后36个月服务费按模板整机基准价和对应基准服务费比例计算，再转为合同币种并加入当地服务VAT。</div></div> : null}</section>
...
function difference(current: number, previous: unknown) {
  ...
  const tone = value > 0 ? "text-[#f56c6c]" : value < 0 ? "text-[#67c23a]" : "text-[#606266]";
