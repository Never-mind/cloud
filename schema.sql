...
CREATE TABLE IF NOT EXISTS `merge_power_requests` (
  ...
  `remoteStatus` VARCHAR(32) NULL COMMENT 'remote fulfillment status',
  `remoteStatusUpdatedAt` DATETIME NULL COMMENT 'remote status update time',
  `remoteCancelledItemCount` INT NOT NULL DEFAULT 0 COMMENT 'remote cancelled item count',
  `cancelReason` VARCHAR(32) NULL COMMENT 'remote_cancelled/local_manual/local_deleted',
  `cancelledAt` DATETIME NULL COMMENT 'cancel time',
  `cancelledByName` VARCHAR(255) NULL COMMENT 'cancel operator',
...
CREATE TABLE IF NOT EXISTS `merge_power_requestitems` (
  ...
  `deviceCode` VARCHAR(64) NOT NULL COMMENT 'device code',
  `requestType` VARCHAR(64) NULL COMMENT 'whole machine/spare parts snapshot',
...
CREATE TABLE IF NOT EXISTS `merge_power_purchaseorderitems` (
  ...
  `requestType` VARCHAR(64) NULL COMMENT 'whole machine/spare parts snapshot',
  `currency` VARCHAR(3) NULL COMMENT 'purchase currency: CNY or USD',
  ...
  `powerPricingJson` LONGTEXT NULL COMMENT 'power service pricing calculation snapshot JSON',
  `powerFirst24VatIncluded` DECIMAL(18, 4) NULL COMMENT 'power service price for first 24 months including VAT',
  `powerNext36VatIncluded` DECIMAL(18, 4) NULL COMMENT 'power service price for next 36 months including VAT',
  `powerFirst24Manual` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'whether first 24 month price is manually overridden',
  `powerNext36Manual` TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'whether next 36 month price is manually overridden',
...
CREATE TABLE IF NOT EXISTS `merge_power_purchaseordersnitems` (
  ...
  `childSparePartCode` TEXT NULL COMMENT 'child spare part code',
  `childTopSn` TEXT NULL COMMENT 'child top SN',
  `customerChildComponentOriginalSn` TEXT NULL COMMENT 'customer child component original SN',
  `childComponentDescription` TEXT NULL COMMENT 'child component description',
  `rowId` TEXT NULL COMMENT 'row id',
  `tenantId` TEXT NULL COMMENT 'tenant id',
  `template` TEXT NULL COMMENT 'template',
  `version` TEXT NULL COMMENT 'version',
  `businessFlowId` TEXT NULL COMMENT 'business flow id',
  `coreDocument` TEXT NULL COMMENT 'core document flag',
  `processName` TEXT NULL COMMENT 'process name',
  `processVersion` TEXT NULL COMMENT 'process version',
  `upstreamDocumentId` TEXT NULL COMMENT 'upstream document id',
  `upstreamDocumentItemId` TEXT NULL COMMENT 'upstream document item id',
  `upstreamDocumentType` TEXT NULL COMMENT 'upstream document type',
  `businessFlowInstanceId` TEXT NULL COMMENT 'business flow instance id',
  `sourceId` TEXT NULL COMMENT 'source id',
  `timestamp` DATETIME NULL COMMENT 'timestamp',
  `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'created time',
  `updatedAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'updated time',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `merge_power_purchaseorderplanitems` (
  ...
  `materialName` TEXT NULL COMMENT 'material name',
  `wholeMachineSupplierPoActivatedAt` DATETIME NULL COMMENT 'whole machine supplier PO activated time',
  `wholeMachineSupplierPoConfirmedAt` DATETIME NULL COMMENT 'whole machine supplier PO confirmed time',
  `requestedQuantity` DECIMAL(18, 4) NULL COMMENT 'requested quantity',
  `formFactor` TEXT NULL COMMENT 'form factor',
  `unit` TEXT NULL COMMENT 'unit',
  `batch` TEXT NULL COMMENT 'batch',
  `sn` VARCHAR(255) NULL COMMENT 'SN',
  `shippedQuantity` DECIMAL(18, 4) NULL COMMENT 'shipped quantity',
  `remark` TEXT NULL COMMENT 'remark',
  `pendingShipmentQuantity` DECIMAL(18, 4) NULL COMMENT 'pending shipment quantity',
  `shippedTotalQuantity` DECIMAL(18, 4) NULL COMMENT 'shipped total quantity',
  `templateIdentifier` TEXT NULL COMMENT 'template identifier',
  `poStatus` TEXT NULL COMMENT 'PO status',
  `rowId` TEXT NULL COMMENT 'row id',
  `headerId` TEXT NULL COMMENT 'header id',
  `slMaterialCode` TEXT NULL COMMENT 'SL material code',
  `codeDescription` TEXT NULL COMMENT 'code description',
  `modelType` TEXT NULL COMMENT 'model type',
  `productName` TEXT NULL COMMENT 'product name',
  `belongingModelName` TEXT NULL COMMENT 'belonging model name',
  `totalModelQuantity` DECIMAL(18, 4) NULL COMMENT 'total model quantity',
  `requestedAt` DATETIME NULL COMMENT 'requested time',
  `matchedQuantity` DECIMAL(18, 4) NULL COMMENT 'matched quantity',
  `supplyDate` DATETIME NULL COMMENT 'supply date',
  `cabinetNodeCode` TEXT NULL COMMENT 'cabinet node code',
  `atpOrder` TEXT NULL COMMENT 'ATP order',
  `orderPriority` TEXT NULL COMMENT 'order priority',
  `datacenterOwner` TEXT NULL COMMENT 'datacenter owner',
  `documentNoDescription` TEXT NULL COMMENT 'document number description',
  `computeMode` TEXT NULL COMMENT 'compute mode',
  `productType` TEXT NULL COMMENT 'product type',
  `category` TEXT NULL COMMENT 'category',
  `dataCenter` TEXT NULL COMMENT 'data center',
  `customizationFlag` TEXT NULL COMMENT 'customization flag',
  `supplyType` TEXT NULL COMMENT 'supply type',
  `supplyInformation` TEXT NULL COMMENT 'supply information',
  `dssEpd` DATETIME NULL COMMENT 'DSS EPD',
  `crd` DATETIME NULL COMMENT 'CRD',
  `rsd` DATETIME NULL COMMENT 'RSD',
  `rpd` DATETIME NULL COMMENT 'RPD',
  `cpd` DATETIME NULL COMMENT 'CPD',
  `esd` DATETIME NULL COMMENT 'ESD',
  `eta` DATETIME NULL COMMENT 'ETA',
  `deliveryContact` TEXT NULL COMMENT 'delivery contact',
  `deliveryContactPhone` TEXT NULL COMMENT 'delivery contact phone',
  `computeSupplier` TEXT NULL COMMENT 'compute supplier',
  `country` TEXT NULL COMMENT 'country',
  `province` TEXT NULL COMMENT 'province',
  `procurementFulfillmentManager` TEXT NULL COMMENT 'procurement fulfillment manager',
  `supplierCode` TEXT NULL COMMENT 'supplier code',
  `odmSupplierCodeV` TEXT NULL COMMENT 'ODM supplier code V',
  `batchNo` TEXT NULL COMMENT 'batch number',
  `city` TEXT NULL COMMENT 'city',
  `deliveryAddress` TEXT NULL COMMENT 'delivery address',
  `urgentOrder` TEXT NULL COMMENT 'urgent order flag',
  `waybillNo` TEXT NULL COMMENT 'waybill number',
  `logisticsCurrentStatus` TEXT NULL COMMENT 'logistics current status',
  `logisticsArrivalTransferAt` DATETIME NULL COMMENT 'logistics arrival transfer time',
  `firstDeliveryAt` DATETIME NULL COMMENT 'first delivery time',
  `apd` DATETIME NULL COMMENT 'APD',
  `asd` DATETIME NULL COMMENT 'ASD',
  `computeSupplierInstructionReceivedAt` DATETIME NULL COMMENT 'compute supplier instruction received time',
  `supplierUnsatisfiedExplanation` TEXT NULL COMMENT 'supplier unsatisfied explanation',
  `firstDeliveryFailureReason` TEXT NULL COMMENT 'first delivery failure reason',
  `productionProgress` TEXT NULL COMMENT 'production progress',
  `supplierFeedbackEsd` DATETIME NULL COMMENT 'supplier feedback ESD',
  `supplierFeedbackEta` DATETIME NULL COMMENT 'supplier feedback ETA',
  `price` DECIMAL(18, 4) NULL COMMENT 'price',
  `purchaseOrderItemRelation` TEXT NULL COMMENT 'purchase order item relation',
  `apdAt` DATETIME NULL COMMENT 'APD time',
  `asdAt` DATETIME NULL COMMENT 'ASD time',
  `rsd2` DATETIME NULL COMMENT 'RSD2',
  `transportMode` TEXT NULL COMMENT 'transport mode',
  `purchaseOrderNo` TEXT NULL COMMENT 'purchase order number',
  `weight` DECIMAL(18, 4) NULL COMMENT 'weight',
  `volume` DECIMAL(18, 4) NULL COMMENT 'volume',
  `pieceCount` DECIMAL(18, 4) NULL COMMENT 'piece count',
  `supplierPoActivatedAt` DATETIME NULL COMMENT 'supplier PO activated time',
  `fulfillmentUnit` TEXT NULL COMMENT 'fulfillment unit',
  `odmSupplierCode` TEXT NULL COMMENT 'ODM supplier code',
  `odmSupplierName` TEXT NULL COMMENT 'ODM supplier name',
  `tenantId` TEXT NULL COMMENT 'tenant id',
  `template` TEXT NULL COMMENT 'template',
  `version` TEXT NULL COMMENT 'version',
  `businessFlowId` TEXT NULL COMMENT 'business flow id',
  `coreDocument` TEXT NULL COMMENT 'core document flag',
  `logisticsName` TEXT NULL COMMENT 'logistics name',
  `processVersion` TEXT NULL COMMENT 'process version',
  `upstreamDocumentItemId` TEXT NULL COMMENT 'upstream document item id',
  `upstreamDocumentType` TEXT NULL COMMENT 'upstream document type',
  `businessFlowInstanceId` TEXT NULL COMMENT 'business flow instance id',
  `timestamp` DATETIME NULL COMMENT 'timestamp',
  `originalRequestedQuantity` DECIMAL(18, 4) NULL COMMENT 'original requested quantity',
  `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'created time',
  `updatedAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'updated time',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `merge_power_servicefeesnapshots` (
  ...
  `repaymentAmount` DECIMAL(18, 4) NULL COMMENT 'repayment amount VAT included',
  `repaymentAmountExcludingTax` DECIMAL(18, 4) NULL COMMENT 'repayment amount VAT excluded',
  `repaymentVatRate` DECIMAL(10, 6) NULL COMMENT 'repayment VAT rate',
  `repaymentDate` DATE NULL COMMENT 'repayment date',
  `repaymentUpdatedAt` DATETIME NULL COMMENT 'repayment updated time',
  `invoiceNo` VARCHAR(128) NULL COMMENT 'invoice number',
  `invoiceCurrency` VARCHAR(16) NULL COMMENT 'invoice currency',
  `invoiceReceivingUnitId` VARCHAR(64) NULL COMMENT 'invoice receiving undertaking unit id',
  `invoicePayerCustomerId` VARCHAR(64) NULL COMMENT 'invoice payer customer id',
