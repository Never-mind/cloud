...
数量、金额、汇率、税率这类输入统一用 `src/components/number-input.tsx` 的 `NumberInput`，不要手写 `<Input type="number">` 再在 `onChange` 里转数字。

手写写法有两个必然出现的缺陷：

1. **小于 1 的小数打不出来**。每敲一个字符就 `Number()` 再 `String()` 回填，`0.0` 会被还原成 `0`，汇率 `0.057` 只能先输 `0.57` 再回头补 0。
2. **残留前导 0**。字段值为 `0` 时输入框显示 `0`，用户接着输入 `88` 得到 `088`；保存后按数字解析虽然是对的，但显示很难看。

```tsx
// 业务自己决定怎么转数字：存字符串就直接存，存数字就 Number(text)
<NumberInput className="w-full" value={draft.amount} onChange={(text) => onDraftChange("amount", text)} />
<NumberInput value={line.unitPrice} onChange={(text) => updateLine(line.id, { unitPrice: Number(text) || 0 })} />
```

- 输入过程中保留用户原文，失焦时才按外部值规范化，所以 `0.`、`0.0`、`0.057` 都能正常敲出来。
- 前导 0 在输入时就被去掉（`088` → `88`、`00.5` → `0.5`、`-007` → `-7`），`0`、`0.`、`0.05` 保持原样。
- 值为 `0`、`null`、`undefined`、空串时显示为空，避免出现"原位置有个 0"。
- `onChange` 抛出的是**原始文本**，不是数字。
- 需要嵌进自定义边框（例如价格卡里的透明输入）时传 `bare`，关闭内置外观，只保留输入行为。
- 纯函数在 `src/lib/numeric-input.ts`，有单元测试覆盖。
...
| 数字输入 | ✅ 新增 `NumberInput` 组件（见 8.5），边输入边保留原文，去掉前导 0、0 显示为空；替换 20 处手写数字输入框 |
| 确认类按钮 | ✅ 请求进行中显示"确认中..."，失败时提示后端原因，不再出现"按钮显示已确认、数据仍是草稿"的误判 |
| 远端不可用降级 | ✅ 远端拉取失败不再阻断确认采购：物流单先生成并标记 `pending`（待远端补全），远端恢复后用"补齐待补全物流"按需求单号补回机房/地址/收件人并列出差异 |
| 物流导入列 | ✅ 修复"机房/收货地址/收件人"三列被静默丢弃的问题：填文本按展示值保存，填档案 ID/名称则解析档案；运输方式与 8 个时间节点列本就支持导入。**新增模板列可导入性回归测试**（覆盖全部实体） |
| 远端时间节点接口 | 🟡 预留 `remote-shipment-timeline.ts`：远端时间节点在另一个模块、目前连不上也没有字段映射，映射表先留空；补齐流程已接好，填入映射即可生效，且遵守"远端有值即以远端为准" |
