...
2. 列表加载态改骨架屏，空状态做成"图标 + 说明 + 主操作"的标准组件。
3. `Panel` 圆角与控件圆角统一（需先处理结差页 `sticky bottom-4` 浮动条与 `overflow: hidden` 的冲突）。
4. 原生 `confirm()` / `alert()` 替换为统一确认弹窗组件。
5. 约 125 个符号导出后只在自身文件内使用，可去掉 `export` 收缩模块对外接口。
