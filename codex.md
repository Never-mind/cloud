...
| 财务管理 - 月账单 | 待生成月账单实例、月账单合同、每月核销明细、月账单对账单 |
...
| 月账单合同初始化导入 | 台账ID可为空自动生成；采购明细ID可为空，按需求单号 + PO单号 + 实例编码自动匹配采购明细，并带出数量、币种、单价等信息 |
...
| `POST` | `/api/billing/confirm` | 生成月账单合同和60个月核销明细 |
...
6. 月账单合同生成后产生60个月核销明细：前24个月使用前24个月价格，后36个月使用后36个月价格。
